...
from zhchess import Color, IntoSquareSet, Square
...
class Arrow:
    ...
    def pgn(self) -> str:
        ...
        if self.tail == self.head:
            return f"{color}{zhchess.SQUARE_NAMES[self.tail]}"
        else:
            return f"{color}{zhchess.SQUARE_NAMES[self.tail]}{zhchess.SQUARE_NAMES[self.head]}"

    def __str__(self) -> str:
        return self.pgn()

    def __repr__(self) -> str:
        return f"Arrow({zhchess.SQUARE_NAMES[self.tail].upper()}, {zhchess.SQUARE_NAMES[self.head].upper()}, color={self.color!r})"
    ...
    def from_pgn(cls, pgn: str) -> Arrow:
        ...
        tail = zhchess.parse_square(pgn[:2])
        head = zhchess.parse_square(pgn[2:]) if len(pgn) > 2 else tail
...
def piece(piece: zhchess.Piece, size: Optional[int] = None) -> str:
    """
    Renders the given :class:`zhchess.Piece` as an SVG image.

    >>> import zhchess
    >>> import zhchess.svg
    >>>
    >>> zhchess.svg.piece(zhchess.Piece.from_symbol("R"))  # doctest: +SKIP
...
def board(
    ...
    *,
    ...
    :param arrows: A list of :class:`~zhchess.svg.Arrow` objects, like
        ``[zhchess.svg.Arrow(zhchess.E2, zhchess.E4)]``, or a list of tuples, like
        ``[(zhchess.E2, zhchess.E4)]``. An arrow from a square pointing to the same
        square is drawn as a circle, like ``[(zhchess.E2, zhchess.E2)]``.
    :param fill: A dictionary mapping squares to a colors that they should be
        filled with.
    :param squares: A :class:`zhchess.SquareSet` with selected squares to mark
        with an X.
    ...
    >>> import zhchess.svg
    >>>
    >>> board = zhchess.Board("8/8/8/8/4N3/8/8/8 w - - 0 1")
    >>>
    >>> zhchess.svg.board(
    ...     board,
    ...     fill=dict.fromkeys(board.attacks(zhchess.E4), "#cc0000cc"),
    ...     arrows=[zhchess.svg.Arrow(zhchess.E4, zhchess.F6, color="#0000cccc")],
    ...     squares=zhchess.SquareSet(zhchess.BB_DARK_SQUARES & zhchess.BB_FILE_B),
    ...     size=350,
    ... )  # doctest: +SKIP
    ...
    if board:
        for piece_color in zhchess.COLORS:
            for piece_type in zhchess.PIECE_TYPES:
                if board.pieces_mask(piece_type, piece_color):
                    defs.append(
                        ET.fromstring(
                            PIECES[zhchess.Piece(piece_type, piece_color).symbol()]
                        )
                    )

    squares = zhchess.SquareSet(squares) if squares else zhchess.SquareSet()
    if squares:
            ...
            _attrs(
                {
                    ...
                    "opacity": outer_border_opacity
                    if outer_border_opacity < 1.0
                    else None,
                }
            ),
        )
            ...
            _attrs(
                {
                    ...
                    "opacity": inner_border_opacity
                    if inner_border_opacity < 1.0
                    else None,
                }
            ),
        )
    ...
    if coordinates:
        ...
        for file_index, file_name in enumerate(zhchess.FILE_NAMES):
            x = (
                (file_index if orientation else 7 - file_index) * SQUARE_SIZE
                + inner_border
                + margin
                + outer_border
            )
        ...
        for rank_index, rank_name in enumerate(zhchess.RANK_NAMES):
            y = (
                (7 - rank_index if orientation else rank_index) * SQUARE_SIZE
                + inner_border
                + margin
                + outer_border
            )
    ...
    # Render board.
    for square, bb in enumerate(zhchess.BB_SQUARES):
        ...
        cls = ["square", "light" if zhchess.BB_LIGHT_SQUARES & bb else "dark"]
        ...
        cls.append(zhchess.SQUARE_NAMES[square])
    ...
    # Render check mark.
    if check is not None:
        file_index = zhchess.square_file(check)
        rank_index = zhchess.square_rank(check)
    ...
    for square, bb in enumerate(zhchess.BB_SQUARES):
        ...
        if board is not None:
            ...
            if piece:
                href = f"#{zhchess.COLOR_NAMES[piece.color]}-{zhchess.PIECE_NAMES[piece.piece_type]}"
                ET.SubElement(
                    svg,
                    "use",
                    {
    ...
    for arrow in arrows:
        ...
        tail_file = zhchess.square_file(tail)
        tail_rank = zhchess.square_rank(tail)
        head_file = zhchess.square_file(head)
        head_rank = zhchess.square_rank(head)

        xtail = (
            outer_border
            + margin
            + inner_border
            + (tail_file + 0.5 if orientation else 7.5 - tail_file) * SQUARE_SIZE
        )
        ytail = (
            outer_border
            + margin
            + inner_border
            + (7.5 - tail_rank if orientation else tail_rank + 0.5) * SQUARE_SIZE
        )
        xhead = (
            outer_border
            + margin
            + inner_border
            + (head_file + 0.5 if orientation else 7.5 - head_file) * SQUARE_SIZE
        )
        yhead = (
            outer_border
            + margin
            + inner_border
            + (7.5 - head_rank if orientation else head_rank + 0.5) * SQUARE_SIZE
        )
