...
from zhchess import Color, Square
...
class GameNode(abc.ABC):
    ...
    def __getitem__(self, move: Union[int, zhchess.Move, GameNode]) -> ChildNode:
        try:
    ...
    def __contains__(self, move: Union[int, zhchess.Move, GameNode]) -> bool:
        try:
            self[move]
        except KeyError:
            return False
    ...
    def variation(self, move: Union[int, zhchess.Move, GameNode]) -> ChildNode:
        """
        Gets a child node by either the move or the variation index.
        """
        return self[move]

    def has_variation(self, move: Union[int, zhchess.Move, GameNode]) -> bool:
        """Checks if this node has the given variation."""
        return move in self

    def promote_to_main(self, move: Union[int, zhchess.Move, GameNode]) -> None:
        """Promotes the given *move* to the main variation."""
        variation = self[move]
        self.variations.remove(variation)
        self.variations.insert(0, variation)

    def promote(self, move: Union[int, zhchess.Move, GameNode]) -> None:
    ...
    def demote(self, move: Union[int, zhchess.Move, GameNode]) -> None:
    ...
    def remove_variation(self, move: Union[int, zhchess.Move, GameNode]) -> None:
        """Removes a variation."""
        self.variations.remove(self.variation(move))

    def add_variation(self, move: zhchess.Move, *, comment: str = "", starting_comment: str = "", nags: Iterable[int] = []) -> ChildNode:
    ...
    def add_main_variation(self, move: zhchess.Move, *, comment: str = "", nags: Iterable[int] = []) -> ChildNode:
        """
    ...
    def mainline_moves(self) -> Mainline[zhchess.Move]:
        """Returns an iterable over the main moves after this node."""
        return Mainline(self, lambda node: node.move)

    def add_line(self, moves: Iterable[zhchess.Move], *, comment: str = "", starting_comment: str = "", nags: Iterable[int] = []) -> GameNode:
        """
    ...
    def eval(self) -> Optional[zhchess.engine.PovScore]:
        """
        ...
        if match.group("mate"):
            ...
            score = zhchess.engine.Cp(round(float(match.group("cp")) * 100))

        return zhchess.engine.PovScore(score if turn else -score, turn)
    ...
    def set_eval(self, score: Optional[zhchess.engine.PovScore], depth: Optional[int] = None) -> None:
        """
    ...
    def arrows(self) -> List[zhchess.svg.Arrow]:
        """
        ...
        for match in ARROWS_REGEX.finditer(self.comment):
            for group in match.group("arrows").split(","):
                arrows.append(zhchess.svg.Arrow.from_pgn(group))

        return arrows

    def set_arrows(self, arrows: Iterable[Union[zhchess.svg.Arrow, Tuple[Square, Square]]]) -> None:
        """
        ...
        for arrow in arrows:
            try:
                ...
                arrow = zhchess.svg.Arrow(tail, head)
            except TypeError:
                pass
...
class ChildNode(GameNode):
    ...
    def __init__(self, parent: GameNode, move: zhchess.Move, *, comment: str = "", starting_comment: str = "", nags: Iterable[int] = []) -> None:
    ...
    def _accept_node(self, parent_board: zhchess.Board, visitor: BaseVisitor[ResultT]) -> None:
    ...
    def _accept(self, parent_board: zhchess.Board, visitor: BaseVisitor[ResultT], *, sidelines: bool = True) -> None:
...
class Game(GameNode):
    ...
    def _interactive_viewer(self) -> Any:
        from zhchess._interactive import InteractiveViewer
    ...
    def setup(self, board: Union[zhchess.Board, str]) -> None:
        """
        ...
        try:
            ...
            setup = typing.cast(zhchess.Board, board)
        except AttributeError:
            setup = zhchess.Board(board)  # type: ignore
...
class Headers(MutableMapping[str, str]):
    ...
    def variant(self) -> Type[zhchess.Board]:
        if "Variant" not in self or self.is_chess960() or self.is_wild():
            return zhchess.Board
        else:
            from zhchess.variant import find_variant
...
class BaseVisitor(abc.ABC, Generic[ResultT]):
    ...
    def begin_parse_san(self, board: zhchess.Board, san: str) -> Optional[SkipType]:
        """
    ...
    def parse_san(self, board: zhchess.Board, san: str) -> zhchess.Move:
        """
    ...
    def visit_board(self, board: zhchess.Board) -> None:
        """
...
class BoardBuilder(BaseVisitor[zhchess.Board]):
    """
