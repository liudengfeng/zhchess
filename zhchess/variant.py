...
class SuicideBoard(zhchess.Board):
    ...
    def pin_mask(self, color: zhchess.Color, square: zhchess.Square) -> zhchess.Bitboard:
        return zhchess.BB_ALL

    def _attacked_for_king(self, path: zhchess.Bitboard, occupied: zhchess.Bitboard) -> bool:
        return False

    def checkers_mask(self) -> zhchess.Bitboard:
        return zhchess.BB_EMPTY

    def gives_check(self, move: zhchess.Move) -> bool:
        return False

    def is_into_check(self, move: zhchess.Move) -> bool:
        return False
    ...
    def _material_balance(self) -> int:
        return (zhchess.popcount(self.occupied_co[self.turn]) -
                zhchess.popcount(self.occupied_co[not self.turn]))
    ...
    def has_insufficient_material(self, color: zhchess.Color) -> bool:
        if not self.occupied_co[color]:
            ...
            # captured.
            we_some_on_light = bool(self.occupied_co[color] & zhchess.BB_LIGHT_SQUARES)
            we_some_on_dark = bool(self.occupied_co[color] & zhchess.BB_DARK_SQUARES)
            they_all_on_dark = not (self.occupied_co[not color] & zhchess.BB_LIGHT_SQUARES)
            they_all_on_light = not (self.occupied_co[not color] & zhchess.BB_DARK_SQUARES)
    ...
    def generate_pseudo_legal_moves(self, from_mask: zhchess.Bitboard = zhchess.BB_ALL, to_mask: zhchess.Bitboard = zhchess.BB_ALL) -> Iterator[zhchess.Move]:
        for move in super().generate_pseudo_legal_moves(from_mask, to_mask):
            # Add king promotions.
            if move.promotion == zhchess.QUEEN:
                yield zhchess.Move(move.from_square, move.to_square, zhchess.KING)

            yield move

    def generate_legal_moves(self, from_mask: zhchess.Bitboard = zhchess.BB_ALL, to_mask: zhchess.Bitboard = zhchess.BB_ALL) -> Iterator[zhchess.Move]:
        ...
        for move in self.generate_pseudo_legal_captures():
            if zhchess.BB_SQUARES[move.from_square] & from_mask and zhchess.BB_SQUARES[move.to_square] & to_mask:
                yield move
            found_capture = True
    ...
    def is_legal(self, move: zhchess.Move) -> bool:
...
class AntichessBoard(GiveawayBoard):
    ...
    def reset(self) -> None:
        super().reset()
        self.castling_rights = zhchess.BB_EMPTY
...
class AtomicBoard(zhchess.Board):
    ...
    def has_insufficient_material(self, color: zhchess.Color) -> bool:
        ...
        if zhchess.popcount(self.knights | self.bishops | self.rooks) == 1:
            return True

        # Two knights cannot mate against bare king.
        if self.occupied == self.knights | self.kings:
            return zhchess.popcount(self.knights) <= 2
    ...
    def _attacked_for_king(self, path: zhchess.Bitboard, occupied: zhchess.Bitboard) -> bool:
        ...
        for enemy_king in zhchess.scan_forward(enemy_kings):
            path &= ~zhchess.BB_KING_ATTACKS[enemy_king]
    ...
    def _push_capture(self, move: zhchess.Move, capture_square: zhchess.Square, piece_type: zhchess.PieceType, was_promoted: bool) -> None:
        explosion_radius = zhchess.BB_KING_ATTACKS[move.to_square] & ~self.pawns
        ...
        for explosion in zhchess.scan_forward(explosion_radius):
            self._remove_piece_at(explosion)

    def checkers_mask(self) -> zhchess.Bitboard:
        return zhchess.BB_EMPTY if self._kings_connected() else super().checkers_mask()
    ...
    def status(self) -> zhchess.Status:
        ...
        if self.turn == zhchess.WHITE:
            ...
            status &= ~zhchess.STATUS_NO_BLACK_KING
        if zhchess.popcount(self.checkers_mask()) <= 14:
            status &= ~zhchess.STATUS_TOO_MANY_CHECKERS
        if self._valid_ep_square() is None:
            status &= ~zhchess.STATUS_IMPOSSIBLE_CHECK
        return status


class KingOfTheHillBoard(zhchess.Board):
    ...
    def is_variant_end(self) -> bool:
        return bool(self.kings & zhchess.BB_CENTER)

    def is_variant_win(self) -> bool:
        return bool(self.kings & self.occupied_co[self.turn] & zhchess.BB_CENTER)

    def is_variant_loss(self) -> bool:
        return bool(self.kings & self.occupied_co[not self.turn] & zhchess.BB_CENTER)

    def has_insufficient_material(self, color: zhchess.Color) -> bool:
        return False


class RacingKingsBoard(zhchess.Board):
    ...
    def is_variant_end(self) -> bool:
        if not self.kings & zhchess.BB_RANK_8:
            return False
    ...
    def is_variant_loss(self) -> bool:
        return self.is_variant_end() and not self.kings & self.occupied_co[self.turn] & zhchess.BB_RANK_8
    ...
    def status(self) -> zhchess.Status:
        status = super().status()
        if self.is_check():
            status |= zhchess.STATUS_RACE_CHECK | zhchess.STATUS_TOO_MANY_CHECKERS | zhchess.STATUS_IMPOSSIBLE_CHECK
        if self.turn == zhchess.BLACK and all(self.occupied_co[co] & self.kings & zhchess.BB_RANK_8 for co in zhchess.COLORS):
            status |= zhchess.STATUS_RACE_OVER
        if self.pawns:
            status |= zhchess.STATUS_RACE_MATERIAL
        for color in zhchess.COLORS:
            if zhchess.popcount(self.occupied_co[color] & self.knights) > 2:
                status |= zhchess.STATUS_RACE_MATERIAL
            if zhchess.popcount(self.occupied_co[color] & self.bishops) > 2:
                status |= zhchess.STATUS_RACE_MATERIAL
            if zhchess.popcount(self.occupied_co[color] & self.rooks) > 2:
                status |= zhchess.STATUS_RACE_MATERIAL
            if zhchess.popcount(self.occupied_co[color] & self.queens) > 1:
                status |= zhchess.STATUS_RACE_MATERIAL
        return status
...
class HordeBoard(zhchess.Board):
    ...
    def has_insufficient_material(self, color: zhchess.Color) -> bool:
        ...
        bishops = zhchess.popcount(white & self.bishops)
        knights = zhchess.popcount(white & self.knights)
        ...
        pieces = self.occupied_co[zhchess.BLACK]
        pieces_pawns = zhchess.popcount(pieces & self.pawns)
        pieces_bishops = zhchess.popcount(pieces & self.bishops)
        pieces_knights = zhchess.popcount(pieces & self.knights)
        pieces_rooks = zhchess.popcount(pieces & self.rooks)
        pieces_queens = zhchess.popcount(pieces & self.queens)
        pieces_darkb = zhchess.popcount(zhchess.BB_DARK_SQUARES & pieces & self.bishops)
        pieces_lightb = zhchess.popcount(zhchess.BB_LIGHT_SQUARES & pieces & self.bishops)
            ...
            if pieces_num == 1:
                ...
                return not (
                    ...
                    pieces_num >= 4 and (
                        ...
                        (has_bishop_pair(zhchess.BLACK) and pieces_pawns >= 1)
                    ) and
...
class ThreeCheckBoard(zhchess.Board):
    ...
    def push(self, move: zhchess.Move) -> None:
        super().push(move)
        if self.is_check():
    ...
    def set_epd(self, epd: str) -> Dict[str, Union[None, str, int, float, zhchess.Move, List[zhchess.Move]]]:
    ...
    def is_irreversible(self, move: zhchess.Move) -> bool:
...
class CrazyhousePocket:
    ...
    def __init__(self, symbols: Iterable[str] = "") -> None:
        ...
        for symbol in symbols:
            self.add(zhchess.PIECE_SYMBOLS.index(symbol))

    def reset(self) -> None:
        """Clears the pocket."""
        self._pieces = [-1, 0, 0, 0, 0, 0, 0]

    def add(self, piece_type: zhchess.PieceType) -> None:
        """Adds a piece of the given type to this pocket."""
        self._pieces[piece_type] += 1

    def remove(self, piece_type: zhchess.PieceType) -> None:
        """Removes a piece of the given type from this pocket."""
        assert self._pieces[piece_type], f"cannot remove {zhchess.piece_symbol(piece_type)} from {self!r}"
        self._pieces[piece_type] -= 1

    def count(self, piece_type: zhchess.PieceType) -> int:
        """Returns the number of pieces of the given type in the pocket."""
        return self._pieces[piece_type]

    def __str__(self) -> str:
        return "".join(zhchess.piece_symbol(pt) * self.count(pt) for pt in reversed(zhchess.PIECE_TYPES))
...
class CrazyhouseBoard(zhchess.Board):
    ...
    def _push_capture(self, move: zhchess.Move, capture_square: zhchess.Square, piece_type: zhchess.PieceType, was_promoted: bool) -> None:
        if was_promoted:
            self.pockets[self.turn].add(zhchess.PAWN)
        else:
    ...
    def legal_drop_squares_mask(self) -> zhchess.Bitboard:
        ...
        if not king_attackers:
            return ~self.occupied
        elif zhchess.popcount(king_attackers) == 1:
            return zhchess.between(king, zhchess.msb(king_attackers)) & ~self.occupied
        else:
            return zhchess.BB_EMPTY

    def legal_drop_squares(self) -> zhchess.SquareSet:
        """
    ...
    def is_pseudo_legal(self, move: zhchess.Move) -> bool:
        if move.drop and move.from_square == move.to_square:
            return (
                move.drop != zhchess.KING and
                not zhchess.BB_SQUARES[move.to_square] & self.occupied and
                not (move.drop == zhchess.PAWN and zhchess.BB_SQUARES[move.to_square] & zhchess.BB_BACKRANKS) and
    ...
    def is_legal(self, move: zhchess.Move) -> bool:
        if move.drop:
            return self.is_pseudo_legal(move) and bool(self.legal_drop_squares_mask() & zhchess.BB_SQUARES[move.to_square])
        else:
            return super().is_legal(move)

    def generate_pseudo_legal_drops(self, to_mask: zhchess.Bitboard = zhchess.BB_ALL) -> Iterator[zhchess.Move]:
        for pt in zhchess.PIECE_TYPES:
            if self.pockets[self.turn].count(pt):
                for to_square in zhchess.scan_forward(to_mask & ~self.occupied & (~zhchess.BB_BACKRANKS if pt == zhchess.PAWN else zhchess.BB_ALL)):
                    yield zhchess.Move(to_square, to_square, drop=pt)

    def generate_legal_drops(self, to_mask: zhchess.Bitboard = zhchess.BB_ALL) -> Iterator[zhchess.Move]:
    ...
    def parse_san(self, san: str) -> zhchess.Move:
        if "@" in san:
            uci = san.rstrip("+#")
            if uci[0] == "@":
                uci = "P" + uci
            move = zhchess.Move.from_uci(uci)
            if not self.is_legal(move):
                raise zhchess.IllegalMoveError(f"illegal drop san: {san!r} in {self.fen()}")
            return move
        else:
    ...
    def has_insufficient_material(self, color: zhchess.Color) -> bool:
        ...
        return (
            zhchess.popcount(self.occupied) + sum(len(pocket) for pocket in self.pockets) <= 3 and
            not self.promoted and
            not self.pawns and
            not self.rooks and
            not self.queens and
            not any(pocket.count(zhchess.PAWN) for pocket in self.pockets) and
            not any(pocket.count(zhchess.ROOK) for pocket in self.pockets) and
            not any(pocket.count(zhchess.QUEEN) for pocket in self.pockets))
...
VARIANTS: List[Type[zhchess.Board]] = [
    zhchess.Board,
...
def find_variant(name: str) -> Type[zhchess.Board]:
    """
