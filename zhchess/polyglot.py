...
class ZobristHasher:
    ...
    def hash_board(self, board: zhchess.BaseBoard) -> int:
        zobrist_hash = 0

        for pivot, squares in enumerate(board.occupied_co):
            for square in zhchess.scan_reversed(squares):
                piece_index = (typing.cast(zhchess.PieceType, board.piece_type_at(square)) - 1) * 2 + pivot
                zobrist_hash ^= self.array[64 * piece_index + square]

        return zobrist_hash

    def hash_castling(self, board: zhchess.Board) -> int:
        zobrist_hash = 0
        ...
        if board.has_queenside_castling_rights(zhchess.BLACK):
            zobrist_hash ^= self.array[768 + 3]

        return zobrist_hash

    def hash_ep_square(self, board: zhchess.Board) -> int:
        ...
        if board.ep_square:
            ...
            ep_mask = zhchess.shift_left(ep_mask) | zhchess.shift_right(ep_mask)

            if ep_mask & board.pawns & board.occupied_co[board.turn]:
                return self.array[772 + zhchess.square_file(board.ep_square)]
        return 0
...
def zobrist_hash(board: zhchess.Board, *, _hasher: Callable[[zhchess.Board], int] = ZobristHasher(POLYGLOT_RANDOM_ARRAY)) -> int:
    """
...
class MemoryMappedReader:
    ...
    def __getitem__(self, index: int) -> Entry:
        ...
        move = zhchess.Move(from_square, to_square, promotion, drop)
    ...
    def find_all(self, board: Union[zhchess.Board, int], *, minimum_weight: int = 1, exclude_moves: Container[zhchess.Move] = []) -> Iterator[Entry]:
        """Seeks a specific position and yields corresponding entries."""
        try:
            key = int(board)  # type: ignore
            context: Optional[zhchess.Board] = None
        except (TypeError, ValueError):
            context = typing.cast(zhchess.Board, board)
    ...
    def find(self, board: Union[zhchess.Board, int], *, minimum_weight: int = 1, exclude_moves: Container[zhchess.Move] = []) -> Entry:
        """
    ...
    def get(self, board: Union[zhchess.Board, int], default: Optional[Entry] = None, *, minimum_weight: int = 1, exclude_moves: Container[zhchess.Move] = []) -> Optional[Entry]:
        try:
            return self.find(board, minimum_weight=minimum_weight, exclude_moves=exclude_moves)
        except IndexError:
            return default

    def choice(self, board: Union[zhchess.Board, int], *, minimum_weight: int = 1, exclude_moves: Container[zhchess.Move] = [], random: Optional[random.Random] = None) -> Entry:
        """
    ...
    def weighted_choice(self, board: Union[zhchess.Board, int], *, exclude_moves: Container[zhchess.Move] = [], random: Optional[random.Random] = None) -> Entry:
        """
