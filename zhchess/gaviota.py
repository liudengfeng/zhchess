...
ITOSQ = [
    zhchess.H7, zhchess.G7, zhchess.F7, zhchess.E7,
    zhchess.H6, zhchess.G6, zhchess.F6, zhchess.E6,
    zhchess.H5, zhchess.G5, zhchess.F5, zhchess.E5,
    zhchess.H4, zhchess.G4, zhchess.F4, zhchess.E4,
    zhchess.H3, zhchess.G3, zhchess.F3, zhchess.E3,
    zhchess.H2, zhchess.G2, zhchess.F2, zhchess.E2,
    zhchess.D7, zhchess.C7, zhchess.B7, zhchess.A7,
    zhchess.D6, zhchess.C6, zhchess.B6, zhchess.A6,
    zhchess.D5, zhchess.C5, zhchess.B5, zhchess.A5,
    zhchess.D4, zhchess.C4, zhchess.B4, zhchess.A4,
    zhchess.D3, zhchess.C3, zhchess.B3, zhchess.A3,
    zhchess.D2, zhchess.C2, zhchess.B2, zhchess.A2,
]
...
def flip_type(x: zhchess.Square, y: zhchess.Square) -> int:
    ret = 0

    if zhchess.square_file(x) > 3:
        x = flip_we(x)
        y = flip_we(y)
        ret |= 1

    if zhchess.square_rank(x) > 3:
        x = flip_ns(x)
        y = flip_ns(y)
        ret |= 2

    rowx = zhchess.square_rank(x)
    colx = zhchess.square_file(x)
    ...
    rowy = zhchess.square_rank(y)
    coly = zhchess.square_file(y)
...
def init_pp48_idx() -> Tuple[List[List[int]], List[int], List[int]]:
    ...
    idx = 0
    for a in range(zhchess.H7, zhchess.A2 - 1, -1):
        for b in range(a - 1, zhchess.A2 - 1, -1):
...
def norm_kkindex(x: zhchess.Square, y: zhchess.Square) -> Tuple[int, int]:
    if zhchess.square_file(x) > 3:
        x = flip_we(x)
        y = flip_we(y)
...
def init_kkidx() -> Tuple[List[List[int]], List[int], List[int]]:
    ...
    for x in range(64):
        for y in range(64):
            ...
            if x != y and not zhchess.BB_KING_ATTACKS[x] & zhchess.BB_SQUARES[y]:
                # Normalize.
...
def kapkb_pctoindex(c: Request) -> int:
    ...
    if not (zhchess.A2 <= pawn < zhchess.A8):
        return NOINDEX
...
class Request:
    ...
    def __init__(self, white_squares: List[int], white_types: List[zhchess.PieceType], black_squares: List[int], black_types: List[zhchess.PieceType], side: int, epsq: int):
...
class PythonTablebase:
    ...
    def probe_dtm(self, board: zhchess.Board) -> int:
        """
    ...
    def get_dtm(self, board: zhchess.Board, default: Optional[int] = None) -> Optional[int]:
        try:
    ...
    def _setup_tablebase(self, req: Request) -> BinaryIO:
        white_letters = "".join(zhchess.piece_symbol(i) for i in req.white_types)
        black_letters = "".join(zhchess.piece_symbol(i) for i in req.black_types)
        ...
        if req.epsq != NOSQUARE:
            ...
            if (j > -1) and (ys[j] == xed):
                ...
                for i in range(len(xs)):
                    if xp[i] == zhchess.PAWN and (xs[i] == capturer_a or xs[i] == capturer_b):
                        epscore = iFORBID
...
class NativeTablebase:
    ...
    def _probe_hard(self, board: zhchess.Board, wdl_only: bool = False) -> int:
        ...
        i = -1
        for i, square in enumerate(zhchess.SquareSet(board.occupied_co[zhchess.BLACK])):
            c_bs[i] = square
            c_bp[i] = typing.cast(zhchess.PieceType, board.piece_type_at(square))
        ...
        if ret and info.value == 2:
            return dtm if board.turn == zhchess.BLACK else -dtm
