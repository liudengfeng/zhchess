...
class SquareTestCase(unittest.TestCase):
    ...
    def test_parse_square(self):
        ...
        self.assertEqual(zhchess.parse_square("i10"), 89)
...
class MoveTestCase(unittest.TestCase):
    def test_equality(self):
        a = zhchess.Move(zhchess.A1, zhchess.A2)
        b = zhchess.Move(zhchess.A1, zhchess.A2)
        c = zhchess.Move(zhchess.H7, zhchess.H8, zhchess.BISHOP)
        d1 = zhchess.Move(zhchess.H7, zhchess.H8)
        d2 = zhchess.Move(zhchess.H7, zhchess.H8)
    ...
    def test_uci_parsing(self):
        self.assertEqual(zhchess.Move.from_uci("b5c7").uci(), "b5c7")
        self.assertEqual(zhchess.Move.from_uci("e7e8a").uci(), "e7e8a")
        self.assertEqual(zhchess.Move.from_uci("P@e4").uci(), "P@e4")
        self.assertEqual(zhchess.Move.from_uci("B@f4").uci(), "B@f4")
        self.assertEqual(zhchess.Move.from_uci("0000").uci(), "0000")

    def test_invalid_uci(self):
        with self.assertRaises(zhchess.InvalidMoveError):
            zhchess.Move.from_uci("")

        with self.assertRaises(zhchess.InvalidMoveError):
            zhchess.Move.from_uci("N")

        with self.assertRaises(zhchess.InvalidMoveError):
            zhchess.Move.from_uci("z1g3")

        with self.assertRaises(zhchess.InvalidMoveError):
            zhchess.Move.from_uci("Q@g9")

    def test_xboard_move(self):
        self.assertEqual(zhchess.Move.from_uci("b5c7").xboard(), "b5c7")
        self.assertEqual(zhchess.Move.from_uci("e7e8a").xboard(), "e7e8a")
        self.assertEqual(zhchess.Move.from_uci("P@e4").xboard(), "P@e4")
        self.assertEqual(zhchess.Move.from_uci("B@f4").xboard(), "B@f4")
        self.assertEqual(zhchess.Move.from_uci("0000").xboard(), "@@@@")

    def test_copy(self):
        a = zhchess.Move.from_uci("N@f3")
        b = zhchess.Move.from_uci("a1h8")
        c = zhchess.Move.from_uci("g7g8r")
...
class PieceTestCase(unittest.TestCase):
    ...
    def test_from_symbol(self):
        red_knight = zhchess.Piece.from_symbol("N")

        self.assertEqual(red_knight.color, zhchess.RED)
        self.assertEqual(red_knight.piece_type, zhchess.KNIGHT)
        self.assertEqual(red_knight.symbol(), "N")
        self.assertEqual(str(red_knight), "N")

        black_queen = zhchess.Piece.from_symbol("a")

        self.assertEqual(black_queen.color, zhchess.BLACK)
        self.assertEqual(black_queen.piece_type, zhchess.QUEEN)
        self.assertEqual(black_queen.symbol(), "a")
        self.assertEqual(str(black_queen), "a")

    def test_hash(self):
        pieces = {zhchess.Piece.from_symbol(symbol) for symbol in "pcrnbakPCRNBAK"}
        self.assertEqual(len(pieces), 14)
        hashes = {hash(piece) for piece in pieces}
        self.assertEqual(hashes, set(range(14)))
...
class BoardTestCase(unittest.TestCase):
    ...
    def test_empty(self):
        ...
        self.assertEqual(board, zhchess.Board(None))
...
#         board.push(zhchess.Move.from_uci("f3d2"))
...
#         board = zhchess.Board(xfen, chess960=True)
#         self.assertEqual(board.castling_rights, zhchess.BB_G1 | zhchess.BB_A8 | zhchess.BB_G8)
...
#         self.assertTrue(board.clean_castling_rights() & zhchess.BB_A1)
...
#         board.remove_piece_at(zhchess.E2)
#         self.assertEqual(board.piece_at(zhchess.E2), None)

#         board.set_piece_at(zhchess.E4, zhchess.Piece.from_symbol("r"))
#         self.assertEqual(board.piece_type_at(zhchess.E4), zhchess.ROOK)

#         board.set_piece_at(zhchess.F1, None)
#         self.assertEqual(board.piece_at(zhchess.F1), None)

#         board.set_piece_at(zhchess.H7, zhchess.Piece.from_symbol("Q"), promoted=True)
#         self.assertEqual(board.promoted, zhchess.BB_H7)

#         board.set_piece_at(zhchess.H7, None)
#         self.assertEqual(board.promoted, zhchess.BB_EMPTY)
#         self.assertEqual(board.piece_at(zhchess.H7), None)
...
#         # King's Gambit.
#         board.push(zhchess.Move.from_uci("e2e4"))
#         board.push(zhchess.Move.from_uci("e7e5"))
#         board.push(zhchess.Move.from_uci("f2f4"))

#         # Accepted.
#         exf4 = zhchess.Move.from_uci("e5f4")
...
#         board = zhchess.Board("r3k2r/8/8/8/8/8/8/R3K2R w KQkq - 1 1")

#         # Let white castle short.
#         move = board.parse_xboard("O-O")
#         self.assertEqual(move, zhchess.Move.from_uci("e1g1"))
...
#         board = zhchess.Board("4k3/8/8/8/8/8/8/4K2R w K - 0 1")
#         self.assertEqual(board.parse_san("O-O"), zhchess.Move.from_uci("e1g1"))
#         with self.assertRaises(zhchess.IllegalMoveError):
#             board.parse_san("Kg1")
#         with self.assertRaises(zhchess.IllegalMoveError):
#             board.parse_san("Kh1")
...
#         board = zhchess.Board(fen, chess960=True)

#         # Let white do the kingside swap.
#         move = board.parse_san("O-O")
#         self.assertEqual(board.san(move), "O-O")
#         self.assertEqual(board.xboard(move), "O-O")
#         self.assertEqual(move.from_square, zhchess.F1)
#         self.assertEqual(move.to_square, zhchess.G1)
#         self.assertIn(move, board.legal_moves)
#         board.push(move)
#         self.assertEqual(board.shredder_fen(), "3r1k1r/4pp2/8/8/8/8/8/4RRK1 b d - 2 1")

#         # Black can not castle kingside.
#         self.assertNotIn(zhchess.Move.from_uci("e8h8"), board.legal_moves)

#         # Let black castle queenside.
#         move = board.parse_san("O-O-O")
#         self.assertEqual(board.san(move), "O-O-O")
#         self.assertEqual(board.xboard(move), "O-O-O")
#         self.assertEqual(move.from_square, zhchess.F8)
#         self.assertEqual(move.to_square, zhchess.D8)
#         self.assertIn(move, board.legal_moves)
#         board.push(move)
...
#         move = board.parse_san("O-O")
#         self.assertEqual(board.san(move), "O-O")
#         self.assertEqual(move.from_square, zhchess.G1)
#         self.assertEqual(move.to_square, zhchess.H1)
#         self.assertIn(move, board.legal_moves)
#         board.push(move)
...
#         board = zhchess.Board("4rrk1/pbbp2p1/1ppnp3/3n1pqp/3N1PQP/1PPNP3/PBBP2P1/4RRK1 w Ff - 10 18", chess960=True)
#         self.assertNotIn(zhchess.Move.from_uci("g1f1"), board.legal_moves)
#         self.assertNotIn(zhchess.Move.from_uci("g1e1"), board.legal_moves)
#         self.assertNotIn(zhchess.Move.from_uci("g1c1"), board.legal_moves)
#         self.assertNotIn(zhchess.Move.from_uci("g1a1"), board.legal_moves)
#         self.assertIn(zhchess.Move.from_uci("g1h1"), board.legal_moves)  # Kh1

#     def test_selective_castling(self):
#         board = zhchess.Board("r3k2r/pppppppp/8/8/8/8/PPPPPPPP/R3K2R w KQkq - 0 1")

#         # King not selected.
#         self.assertFalse(any(board.generate_castling_moves(zhchess.BB_ALL & ~board.kings)))

#         # Rook on h1 not selected.
#         moves = board.generate_castling_moves(zhchess.BB_ALL, zhchess.BB_ALL & ~zhchess.BB_H1)
...
#         # rights away.
#         board = zhchess.Board("2r1k2r/2qbbpp1/p2pp3/1p3PP1/Pn2P3/1PN1B3/1P3QB1/1K1R3R b k - 0 22")
#         board.push_san("Rxh1")
...
#         board = zhchess.Board("4k2r/8/8/8/8/8/8/R1K5 w KQkq - 0 1", chess960=True)
#         self.assertEqual(board.status(), zhchess.STATUS_BAD_CASTLING_RIGHTS)
#         self.assertEqual(board.fen(), "4k2r/8/8/8/8/8/8/R1K5 w Qk - 0 1")

#         board = zhchess.Board("1r2k3/8/1p6/8/8/5P2/8/1R2KR2 w KQkq - 0 1", chess960=True)
#         self.assertEqual(board.status(), zhchess.STATUS_BAD_CASTLING_RIGHTS)
...
#         board = zhchess.Board("4k3/8/8/1b6/8/8/8/5RKR w KQ - 0 1", chess960=True)
#         self.assertFalse(board.is_legal(zhchess.Move.from_uci("g1f1")))

#     def test_find_move(self):
#         board = zhchess.Board("4k3/1P6/8/8/8/8/3P4/4K2R w K - 0 1")

#         # Pawn moves.
#         self.assertEqual(board.find_move(zhchess.D2, zhchess.D4), zhchess.Move.from_uci("d2d4"))
#         self.assertEqual(board.find_move(zhchess.B7, zhchess.B8), zhchess.Move.from_uci("b7b8q"))
#         self.assertEqual(board.find_move(zhchess.B7, zhchess.B8, zhchess.KNIGHT), zhchess.Move.from_uci("b7b8n"))

#         # Illegal moves.
#         with self.assertRaises(zhchess.IllegalMoveError):
#             board.find_move(zhchess.D2, zhchess.D8)
#         with self.assertRaises(zhchess.IllegalMoveError):
#             board.find_move(zhchess.E1, zhchess.A1)

#         # Castling.
#         self.assertEqual(board.find_move(zhchess.E1, zhchess.G1), zhchess.Move.from_uci("e1g1"))
#         self.assertEqual(board.find_move(zhchess.E1, zhchess.H1), zhchess.Move.from_uci("e1g1"))
#         board.chess960 = True
#         self.assertEqual(board.find_move(zhchess.E1, zhchess.H1), zhchess.Move.from_uci("e1h1"))

#     def test_clean_castling_rights(self):
#         board = zhchess.Board()
#         board.set_board_fen("k6K/8/8/pppppppp/8/8/8/QqQq4")
#         self.assertEqual(board.clean_castling_rights(), zhchess.BB_EMPTY)
...
#         _check(zhchess.Board(), False, False)
#         _check(zhchess.Board("k1K1B1B1/8/8/8/8/8/8/8 w - - 7 32"), True, True)
#         _check(zhchess.Board("kbK1B1B1/8/8/8/8/8/8/8 w - - 7 32"), False, False)
#         _check(zhchess.Board("8/5k2/8/8/8/8/3K4/8 w - - 0 1"), True, True)
#         _check(zhchess.Board("8/3k4/8/8/2N5/8/3K4/8 b - - 0 1"), True, True)
#         _check(zhchess.Board("8/4rk2/8/8/8/8/3K4/8 w - - 0 1"), True, False)
#         _check(zhchess.Board("8/4qk2/8/8/8/8/3K4/8 w - - 0 1"), True, False)
#         _check(zhchess.Board("8/4bk2/8/8/8/8/3KB3/8 w - - 0 1"), False, False)
#         _check(zhchess.Board("8/8/3Q4/2bK4/B7/8/1k6/8 w - - 1 68"), False, False)
#         _check(zhchess.Board("8/5k2/8/8/8/4B3/3K1B2/8 w - - 0 1"), True, True)
#         _check(zhchess.Board("5K2/8/8/1B6/8/k7/6b1/8 w - - 0 39"), True, True)
#         _check(zhchess.Board("8/8/8/4k3/5b2/3K4/8/2B5 w - - 0 33"), True, True)
#         _check(zhchess.Board("3b4/8/8/6b1/8/8/R7/K1k5 w - - 0 1"), False, True)

#         _check(zhchess.variant.AtomicBoard("8/3k4/8/8/2N5/8/3K4/8 b - - 0 1"), True, True)
#         _check(zhchess.variant.AtomicBoard("8/4rk2/8/8/8/8/3K4/8 w - - 0 1"), True, True)
#         _check(zhchess.variant.AtomicBoard("8/4qk2/8/8/8/8/3K4/8 w - - 0 1"), True, False)
#         _check(zhchess.variant.AtomicBoard("8/1k6/8/2n5/8/3NK3/8/8 b - - 0 1"), False, False)
#         _check(zhchess.variant.AtomicBoard("8/4bk2/8/8/8/8/3KB3/8 w - - 0 1"), True, True)
#         _check(zhchess.variant.AtomicBoard("4b3/5k2/8/8/8/8/3KB3/8 w - - 0 1"), False, False)
#         _check(zhchess.variant.AtomicBoard("3Q4/5kKB/8/8/8/8/8/8 b - - 0 1"), False, True)
#         _check(zhchess.variant.AtomicBoard("8/5k2/8/8/8/8/5K2/4bb2 w - - 0 1"), True, False)
#         _check(zhchess.variant.AtomicBoard("8/5k2/8/8/8/8/5K2/4nb2 w - - 0 1"), True, False)

#         _check(zhchess.variant.GiveawayBoard("8/4bk2/8/8/8/8/3KB3/8 w - - 0 1"), False, False)
#         _check(zhchess.variant.GiveawayBoard("4b3/5k2/8/8/8/8/3KB3/8 w - - 0 1"), False, False)
#         _check(zhchess.variant.GiveawayBoard("8/8/8/6b1/8/3B4/4B3/5B2 w - - 0 1"), True, True)
#         _check(zhchess.variant.GiveawayBoard("8/8/5b2/8/8/3B4/3B4/8 w - - 0 1"), True, False)
#         _check(zhchess.variant.SuicideBoard("8/5p2/5P2/8/3B4/1bB5/8/8 b - - 0 1"), false_negative, false_negative)
#         _check(zhchess.variant.AntichessBoard("8/8/8/1n2N3/8/8/8/8 w - - 0 32"), True, False)
#         _check(zhchess.variant.AntichessBoard("8/3N4/8/1n6/8/8/8/8 b - - 1 32"), True, False)
#         _check(zhchess.variant.AntichessBoard("6n1/8/8/4N3/8/8/8/8 b - - 0 27"), False, True)
#         _check(zhchess.variant.AntichessBoard("8/8/5n2/4N3/8/8/8/8 w - - 1 28"), False, True)
#         _check(zhchess.variant.AntichessBoard("8/3n4/8/8/8/8/8/8 w - - 0 29"), False, True)

#         _check(zhchess.variant.KingOfTheHillBoard("8/5k2/8/8/8/8/3K4/8 w - - 0 1"), False, False)

#         _check(zhchess.variant.RacingKingsBoard("8/5k2/8/8/8/8/3K4/8 w - - 0 1"), False, False)

#         _check(zhchess.variant.ThreeCheckBoard("8/5k2/8/8/8/8/3K4/8 w - - 3+3 0 1"), True, True)
#         _check(zhchess.variant.ThreeCheckBoard("8/5k2/8/8/8/8/3K2N1/8 w - - 3+3 0 1"), False, True)

#         _check(zhchess.variant.CrazyhouseBoard("8/5k2/8/8/8/8/3K2N1/8[] w - - 0 1"), True, True)
#         _check(zhchess.variant.CrazyhouseBoard("8/5k2/8/8/8/5B2/3KB3/8[] w - - 0 1"), False, False)
#         _check(zhchess.variant.CrazyhouseBoard("8/8/8/8/3k4/3N~4/3K4/8 w - - 0 1"), False, False)

#         _check(zhchess.variant.HordeBoard("8/5k2/8/8/8/4NN2/8/8 w - - 0 1"), True, False)
#         _check(zhchess.variant.HordeBoard("8/1b5r/1P6/1Pk3q1/1PP5/r1P5/P1P5/2P5 b - - 0 52"), False, False)

#     def test_promotion_with_check(self):
#         board = zhchess.Board("8/6P1/2p5/1Pqk4/6P1/2P1RKP1/4P1P1/8 w - - 0 1")
#         board.push(zhchess.Move.from_uci("g7g8q"))
#         self.assertTrue(board.is_check())
#         self.assertEqual(board.fen(), "6Q1/8/2p5/1Pqk4/6P1/2P1RKP1/4P1P1/8 b - - 0 1")

#         board = zhchess.Board("8/8/8/3R1P2/8/2k2K2/3p4/r7 b - - 0 82")
#         board.push_san("d1=Q+")
#         self.assertEqual(board.fen(), "8/8/8/3R1P2/8/2k2K2/8/r2q4 w - - 0 83")

#     def test_ambiguous_move(self):
#         board = zhchess.Board("8/8/1n6/3R1P2/1n6/2k2K2/3p4/r6r b - - 0 82")
#         with self.assertRaises(zhchess.AmbiguousMoveError):
#             board.parse_san("Rf1")
#         with self.assertRaises(zhchess.AmbiguousMoveError):
#             board.parse_san("Nd5")
...
#         e4 = zhchess.Move.from_uci("e2e4")
#         self.assertIn(e4, board.legal_moves)
#         board.push(e4)

#         e5 = zhchess.Move.from_uci("e7e5")
#         self.assertIn(e5, board.legal_moves)
#         board.push(e5)

#         Qf3 = zhchess.Move.from_uci("d1f3")
#         self.assertIn(Qf3, board.legal_moves)
#         board.push(Qf3)

#         Nc6 = zhchess.Move.from_uci("b8c6")
#         self.assertIn(Nc6, board.legal_moves)
#         board.push(Nc6)

#         Bc4 = zhchess.Move.from_uci("f1c4")
#         self.assertIn(Bc4, board.legal_moves)
#         board.push(Bc4)

#         Rb8 = zhchess.Move.from_uci("a8b8")
...
#         Qf7_mate = zhchess.Move.from_uci("f3f7")
...
#         # White checkmated.
#         board = zhchess.Board("rnb1kbnr/pppp1ppp/8/4p3/6Pq/5P2/PPPPP2P/RNBQKBNR w KQkq - 1 3")
#         self.assertEqual(board.result(claim_draw=True), "0-1")

#         # Stalemate.
#         board = zhchess.Board("7K/7P/7k/8/6q1/8/8/8 w - - 0 1")
#         self.assertEqual(board.result(), "1/2-1/2")

#         # Insufficient material.
#         board = zhchess.Board("4k3/8/8/8/8/5B2/8/4K3 w - - 0 1")
#         self.assertEqual(board.result(), "1/2-1/2")

#         # Fiftyseven-move rule.
#         board = zhchess.Board("4k3/8/6r1/8/8/8/2R5/4K3 w - - 369 1")
#         self.assertEqual(board.result(), "1/2-1/2")

#         # Fifty-move rule.
#         board = zhchess.Board("4k3/8/6r1/8/8/8/2R5/4K3 w - - 120 1")
...
#         board = zhchess.Board(fen)
#         long_castle_check = zhchess.Move.from_uci("e1a1")
...
#         board = zhchess.Board(fen)
#         fxe6_mate_ep = zhchess.Move.from_uci("e5d6")
...
#         board = zhchess.Board(fen)
#         self.assertEqual(board.san(zhchess.Move.from_uci("e1f1")), "Kf1")
#         self.assertEqual(board.san(zhchess.Move.from_uci("c3c2")), "Rcc2")
#         self.assertEqual(board.san(zhchess.Move.from_uci("b2c2")), "Rbc2")
#         self.assertEqual(board.san(zhchess.Move.from_uci("a4b6")), "N4b6")
#         self.assertEqual(board.san(zhchess.Move.from_uci("h8g6")), "N8g6")
#         self.assertEqual(board.san(zhchess.Move.from_uci("h4g6")), "Nh4g6")
#         self.assertEqual(board.fen(), fen)

#         # Do not disambiguate illegal alternatives.
#         fen = "8/8/8/R2nkn2/8/8/2K5/8 b - - 0 1"
#         board = zhchess.Board(fen)
#         self.assertEqual(board.san(zhchess.Move.from_uci("f5e3")), "Ne3+")
#         self.assertEqual(board.fen(), fen)

#         # Promotion.
#         fen = "7k/1p2Npbp/8/2P5/1P1r4/3b2QP/3q1pPK/2RB4 b - - 1 29"
#         board = zhchess.Board(fen)
#         self.assertEqual(board.san(zhchess.Move.from_uci("f2f1q")), "f1=Q")
#         self.assertEqual(board.san(zhchess.Move.from_uci("f2f1n")), "f1=N+")
#         self.assertEqual(board.fen(), fen)
...
#         fen = "N3k2N/8/8/3N4/N4N1N/2R5/1R6/4K3 w - - 0 1"
#         board = zhchess.Board(fen)
#         self.assertEqual(board.lan(zhchess.Move.from_uci("e1f1")), "Ke1-f1")
#         self.assertEqual(board.lan(zhchess.Move.from_uci("c3c2")), "Rc3-c2")
#         self.assertEqual(board.lan(zhchess.Move.from_uci("a4c5")), "Na4-c5")
#         self.assertEqual(board.fen(), fen)

#         # Normal capture.
#         fen = "rnbq1rk1/ppp1bpp1/4pn1p/3p2B1/2PP4/2N1PN2/PP3PPP/R2QKB1R w KQ - 0 7"
#         board = zhchess.Board(fen)
#         self.assertEqual(board.lan(zhchess.Move.from_uci("g5f6")), "Bg5xf6")
#         self.assertEqual(board.fen(), fen)

#         # Pawn captures and moves.
#         fen = "6bk/7b/8/3pP3/8/8/8/Q3K3 w - d6 0 2"
#         board = zhchess.Board(fen)
#         self.assertEqual(board.lan(zhchess.Move.from_uci("e5d6")), "e5xd6#")
#         self.assertEqual(board.lan(zhchess.Move.from_uci("e5e6")), "e5-e6+")
#         self.assertEqual(board.fen(), fen)

#     def test_san_newline(self):
#         board = zhchess.Board("rnbqk2r/ppppppbp/5np1/8/8/5NP1/PPPPPPBP/RNBQK2R w KQkq - 2 4")
#         with self.assertRaises(zhchess.InvalidMoveError):
#             board.parse_san("O-O\n")
#         with self.assertRaises(zhchess.InvalidMoveError):
#             board.parse_san("Nc3\n")
...
#         board = zhchess.Board("2rq1rk1/pb2bppp/1p2p3/n1ppPn2/2PP4/PP3N2/1B1NQPPP/RB3RK1 b - - 4 13")
#         with self.assertRaises(zhchess.IllegalMoveError):
#             board.parse_san("c4")
#         board = zhchess.Board("4k3/8/8/4Pp2/8/8/8/4K3 w - f6 0 2")
#         with self.assertRaises(zhchess.IllegalMoveError):
#             board.parse_san("f6")
...
#                          board.variation_san([zhchess.Move.from_uci(m) for m in
#                                               ['e2e4', 'e7e5', 'g1f3']]))
...
#                      'f6e7', 'f8e7']
#         var_w = board.variation_san([zhchess.Move.from_uci(m) for m in variation])
...
#         board.push(zhchess.Move.from_uci(variation.pop(0)))
#         var_b = board.variation_san([zhchess.Move.from_uci(m) for m in variation])
...
#         board = zhchess.Board(fen)
#         with self.assertRaises(zhchess.IllegalMoveError) as err:
#             board.variation_san([zhchess.Move.from_uci(m) for m in illegal_variation])
...
#         board.push_uci("f8d6")
#         board.push_uci("e1h1")
#         san = zhchess.Board().variation_san(board.move_stack)
...
#         self.assertIn(zhchess.Move.from_uci("e1g1"), board.legal_moves)
#         self.assertIn(zhchess.Move.from_uci("g7g8r"), board.legal_moves)

#         # Impossible promotion: Kb5, h7.
#         self.assertNotIn(zhchess.Move.from_uci("a5b5q"), board.legal_moves)
#         self.assertNotIn(zhchess.Move.from_uci("h6h7n"), board.legal_moves)

#         # Missing promotion.
#         self.assertNotIn(zhchess.Move.from_uci("g7g8"), board.legal_moves)

#         # Promote to pawn or king.
#         self.assertFalse(board.is_legal(zhchess.Move.from_uci("g7g8p")))
#         self.assertFalse(board.is_pseudo_legal(zhchess.Move.from_uci("g7g8p")))
#         self.assertFalse(board.is_legal(zhchess.Move.from_uci("g7g8k")))
#         self.assertFalse(board.is_pseudo_legal(zhchess.Move.from_uci("g7g8k")))

#         self.assertEqual(board.fen(), fen)

#     def test_move_count(self):
#         board = zhchess.Board("1N2k3/P7/8/8/3n4/8/2PP4/R3K2R w KQ - 0 1")
...
#         self.assertEqual(zhchess.polyglot.zobrist_hash(board), 0x3c8123ea7b067637)

#         board.push_san("bxc3")
#         board.push_san("Ra3")
#         self.assertEqual(board.fen(), "rnbqkbnr/p1pppppp/8/8/P6P/R1p5/1P1PPPP1/1NBQKBNR b Kkq - 1 4")
#         self.assertEqual(zhchess.polyglot.zobrist_hash(board), 0x5c3f9b829b279560)
...
#         board = zhchess.Board(fen)
#         illegal_move = zhchess.Move.from_uci("g1g2")
...
#         illegal_move = zhchess.Move.from_uci("e1c1")
#         self.assertNotIn(illegal_move, board.pseudo_legal_moves)
...
#         illegal_move = zhchess.Move.from_uci("e8f8")
#         self.assertNotIn(illegal_move, board.pseudo_legal_moves)
#         self.assertNotIn(illegal_move, board.generate_pseudo_legal_moves())
#         self.assertNotIn(illegal_move, board.legal_moves)
...
#         board = zhchess.Board("r1b1k3/p2p1Nr1/n2b3p/3pp1pP/2BB1p2/P3P2R/Q1P3P1/R3K1N1 b Qq - 0 1")
#         count = 0
...
#         board = zhchess.Board("4r3/3k4/8/8/8/8/q5PP/1R1KR3 w Q - 2 2", chess960=True)
#         move = zhchess.Move.from_uci("d1b1")
...
#     def test_equality(self):
#         self.assertEqual(zhchess.Board(), zhchess.Board())
#         self.assertFalse(zhchess.Board() != zhchess.Board())
...
#         board.remove_piece_at(zhchess.H1)
#         self.assertTrue(board.status() & zhchess.STATUS_BAD_CASTLING_RIGHTS)

#         board.remove_piece_at(zhchess.E8)
#         self.assertTrue(board.status() & zhchess.STATUS_NO_BLACK_KING)

#         # The en passant square should be set even if no capture is actually
#         # possible.
#         board = zhchess.Board()
#         board.push_san("e4")
#         self.assertEqual(board.ep_square, zhchess.E3)
#         self.assertEqual(board.status(), zhchess.STATUS_VALID)

#         # But there must indeed be a pawn there.
#         board.remove_piece_at(zhchess.E4)
#         self.assertEqual(board.status(), zhchess.STATUS_INVALID_EP_SQUARE)
...
#         self.assertEqual(board.status(), zhchess.STATUS_VALID)
#         board = zhchess.Board("bbqnnrkr/pppppppp/8/8/8/8/PPPPPPPP/BBQNNRKR w KQkq - 0 1", chess960=False)
#         self.assertEqual(board.status(), zhchess.STATUS_BAD_CASTLING_RIGHTS)

#         # Opposite check.
#         board = zhchess.Board("4k3/8/8/8/8/8/4Q3/4K3 w - - 0 1")
#         self.assertEqual(board.status(), zhchess.STATUS_OPPOSITE_CHECK)

#         # Empty board.
#         board = zhchess.Board(None)
#         self.assertEqual(board.status(), zhchess.STATUS_EMPTY | zhchess.STATUS_NO_WHITE_KING | zhchess.STATUS_NO_BLACK_KING)

#         # Too many kings.
#         board = zhchess.Board("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBKKBNR w KQkq - 0 1")
#         self.assertEqual(board.status(), zhchess.STATUS_TOO_MANY_KINGS)

#         # Triple check.
#         board = zhchess.Board("4k3/5P2/3N4/8/8/8/4R3/4K3 b - - 0 1")
#         self.assertEqual(board.status(), zhchess.STATUS_TOO_MANY_CHECKERS | zhchess.STATUS_IMPOSSIBLE_CHECK)

#         # Impossible checker alignment.
#         board = zhchess.Board("3R4/8/q4k2/2B5/1NK5/3b4/8/8 w - - 0 1")
#         self.assertEqual(board.status(), zhchess.STATUS_IMPOSSIBLE_CHECK)
#         board = zhchess.Board("2Nq4/2K5/1b6/8/7R/3k4/7P/8 w - - 0 1")
#         self.assertEqual(board.status(), zhchess.STATUS_IMPOSSIBLE_CHECK)
#         board = zhchess.Board("5R2/2P5/8/4k3/8/3rK2r/8/8 w - - 0 1")
#         self.assertEqual(board.status(), zhchess.STATUS_IMPOSSIBLE_CHECK)
#         board = zhchess.Board("8/8/8/1k6/3Pp3/8/8/4KQ2 b - d3 0 1")
#         self.assertEqual(board.status(), zhchess.STATUS_IMPOSSIBLE_CHECK)

#         # Checkers aligned with opponent king are fine.
#         board = zhchess.Board("8/8/5k2/p1q5/PP1rp1P1/3P1N2/2RK1r2/5nN1 w - - 0 3")
#         self.assertEqual(board.status(), zhchess.STATUS_VALID)
...
#         epd = board.epd(bm=zhchess.Move(zhchess.D6, zhchess.D1), id="BK.01")
#         self.assertIn(epd, [
#             "1k1r4/pp1b1R2/3q2pp/4p3/2B5/4Q3/PPP2B2/2K5 b - - bm Qd1+; id \"BK.01\";",
#             "1k1r4/pp1b1R2/3q2pp/4p3/2B5/4Q3/PPP2B2/2K5 b - - id \"BK.01\"; bm Qd1+;"])

#         # Create an EPD with a noop.
#         board = zhchess.Board("4k3/8/8/8/8/8/8/4K3 w - - 0 1")
...
#         board = zhchess.Board("k7/8/8/8/8/8/4PPPP/4K1NR w K - 0 1")
#         epd = board.epd(pv=[
#             zhchess.Move.from_uci("g1f3"),  # Nf3
#             zhchess.Move.from_uci("a8a7"),  # Ka7
#             zhchess.Move.from_uci("e1h1"),  # O-O
#         ])
#         self.assertEqual(epd, "k7/8/8/8/8/8/4PPPP/4K1NR w K - pv Nf3 Ka7 O-O;")

#         # Create an EPD with a set of moves.
#         board = zhchess.Board("8/8/8/4k3/8/1K6/8/8 b - - 0 1")
#         epd = board.epd(bm=[
#             zhchess.Move.from_uci("e5e6"),  # Ke6
#             zhchess.Move.from_uci("e5e4"),  # Ke4
#         ])
...
#         self.assertIn(zhchess.Move(zhchess.F2, zhchess.F4), operations["bm"])
...
#         self.assertEqual(operations["test"], zhchess.Move(zhchess.C5, zhchess.E4))
...
#         self.assertIn(zhchess.Move.from_uci("d6b8"), operations["bm"])
#         self.assertIn(zhchess.Move.from_uci("d6e7"), operations["bm"])
...
#         self.assertEqual(operations["pv"][0], zhchess.Move.from_uci("h1h7"))
#         self.assertEqual(operations["pv"][1], zhchess.Move.from_uci("g8f8"))
#         self.assertEqual(operations["pv"][2], zhchess.Move.from_uci("h7f7"))
...
#         self.assertEqual(operations["bm"], [zhchess.Move.from_uci("f3g5")])
...
#         self.assertEqual(operations["bm"], [zhchess.Move.from_uci("d6d1")])
...
#         board, ops = zhchess.Board.from_epd(epd)
#         self.assertEqual(ops["id"], "ERET 001 - Entlastung")
#         self.assertEqual(ops["bm"], [zhchess.Move.from_uci("f1f4")])

#     def test_null_moves(self):
#         self.assertEqual(str(zhchess.Move.null()), "0000")
#         self.assertEqual(zhchess.Move.null().uci(), "0000")
#         self.assertFalse(zhchess.Move.null())

#         fen = "rnbqkbnr/ppp1pppp/8/2Pp4/8/8/PP1PPPPP/RNBQKBNR w KQkq d6 0 2"
#         board = zhchess.Board(fen)

#         self.assertEqual(zhchess.Move.from_uci("0000"), board.push_san("--"))
#         self.assertEqual(board.fen(), "rnbqkbnr/ppp1pppp/8/2Pp4/8/8/PP1PPPPP/RNBQKBNR b KQkq - 1 2")

#         self.assertEqual(zhchess.Move.null(), board.pop())
#         self.assertEqual(board.fen(), fen)
...
#         self.assertIn(zhchess.B2, attackers)
#         self.assertNotIn(zhchess.D4, attackers)
#         self.assertNotIn(zhchess.E1, attackers)

#     def test_en_passant_attackers(self):
#         board = zhchess.Board("4k3/8/8/8/4pPp1/8/8/4K3 b - f3 0 1")

#         # Attacking the en passant square.
#         attackers = board.attackers(zhchess.BLACK, zhchess.F3)
#         self.assertEqual(len(attackers), 2)
#         self.assertIn(zhchess.E4, attackers)
#         self.assertIn(zhchess.G4, attackers)

#         # Not attacking the pawn directly.
#         attackers = board.attackers(zhchess.BLACK, zhchess.F4)
#         self.assertEqual(attackers, zhchess.BB_EMPTY)

#     def test_attacks(self):
#         board = zhchess.Board("5rk1/p5pp/2p3p1/1p1pR3/3P2P1/2N5/PP3n2/2KB4 w - - 1 26")

#         attacks = board.attacks(zhchess.E5)
#         self.assertEqual(len(attacks), 11)
#         self.assertIn(zhchess.D5, attacks)
#         self.assertIn(zhchess.E1, attacks)
#         self.assertIn(zhchess.F5, attacks)
#         self.assertNotIn(zhchess.E5, attacks)
#         self.assertNotIn(zhchess.C5, attacks)
#         self.assertNotIn(zhchess.F4, attacks)

#         pawn_attacks = board.attacks(zhchess.B2)
#         self.assertIn(zhchess.A3, pawn_attacks)
#         self.assertNotIn(zhchess.B3, pawn_attacks)

#         self.assertFalse(board.attacks(zhchess.G1))
...
#         self.assertFalse(board.piece_at(zhchess.E1))
#         self.assertEqual(zhchess.popcount(board.occupied), 0)
...
#         self.assertTrue(zhchess.Board().is_repetition(1))
...
#         board = zhchess.Board("8/5R2/8/r2KB3/6k1/8/8/8 w - - 19 79")
#         self.assertFalse(board.is_fifty_moves())
#         self.assertFalse(board.can_claim_fifty_moves())
#         board = zhchess.Board("8/8/6r1/4B3/8/4K2k/5R2/8 b - - 68 103")
#         self.assertFalse(board.is_fifty_moves())
#         self.assertFalse(board.can_claim_fifty_moves())
#         board = zhchess.Board("6R1/7k/8/8/1r3B2/5K2/8/8 w - - 99 119")
#         self.assertFalse(board.is_fifty_moves())
#         self.assertTrue(board.can_claim_fifty_moves())
#         board = zhchess.Board("8/7k/8/6R1/1r3B2/5K2/8/8 b - - 100 119")
#         self.assertTrue(board.is_fifty_moves())
#         self.assertTrue(board.can_claim_fifty_moves())
#         board = zhchess.Board("8/7k/8/1r3KR1/5B2/8/8/8 w - - 105 122")
#         self.assertTrue(board.is_fifty_moves())
#         self.assertTrue(board.can_claim_fifty_moves())

#         # Once checkmated, it is too late to claim.
#         board = zhchess.Board("k7/8/NKB5/8/8/8/8/8 b - - 105 176")
#         self.assertFalse(board.is_fifty_moves())
#         self.assertFalse(board.can_claim_fifty_moves())

#         # A stalemate is a draw, but you can not and do not need to claim it by
#         # the fifty-move rule.
#         board = zhchess.Board("k7/3N4/1K6/1B6/8/8/8/8 b - - 99 1")
...
#         same_board = zhchess.Board(board.fen())
#         self.assertEqual(board, same_board)

#     def test_ep_legality(self):
#         move = zhchess.Move.from_uci("h5g6")
#         board = zhchess.Board("rnbqkbnr/pppppp2/7p/6pP/8/8/PPPPPPP1/RNBQKBNR w KQkq g6 0 3")
#         self.assertTrue(board.is_legal(move))
#         board.push_san("Nf3")
...
#         move = zhchess.Move.from_uci("c4d3")
#         board = zhchess.Board("rnbqkbnr/pp1ppppp/8/8/2pP4/2P2N2/PP2PPPP/RNBQKB1R b KQkq d3 0 3")
#         self.assertTrue(board.is_legal(move))
#         board.push_san("Qc7")
#         self.assertFalse(board.is_legal(move))
#         board.push_san("Bd2")
...
#         sample_moves = [
#             zhchess.Move(zhchess.A2, zhchess.A4),
#             zhchess.Move(zhchess.C1, zhchess.E3),
#             zhchess.Move(zhchess.G8, zhchess.F6),
#             zhchess.Move(zhchess.D7, zhchess.D8, zhchess.QUEEN),
#             zhchess.Move(zhchess.E5, zhchess.E4),
#         ]

#         sample_fens = [
#             zhchess.STARTING_FEN,
...
#             board = zhchess.Board(sample_fen)
...
#         board = zhchess.Board("r3k2r/8/8/8/8/8/8/R3K2R w KQkq - 0 1")
#         kingside = zhchess.Move.from_uci("e1g1")
#         queenside = zhchess.Move.from_uci("e1c1")

#         moves = list(board.generate_pseudo_legal_moves())
#         self.assertIn(kingside, moves)
#         self.assertIn(queenside, moves)

#         moves = list(board.generate_pseudo_legal_moves(from_mask=zhchess.BB_RANK_2))
#         self.assertEqual(moves, [])

#         moves = list(board.generate_pseudo_legal_moves(to_mask=zhchess.BB_A1))
...
#         board = zhchess.Board("7k/1p1qn1b1/pB1p1n2/3Pp3/4Pp1p/2QN1B2/PP4PP/6K1 w - - 0 28")
...
#         board = zhchess.Board("r1bqkb1r/p3np2/2n1p2p/1p4pP/2pP4/4PQ1N/1P2BPP1/RNB1K2R w KQkq g6 0 11")
...
#         board = zhchess.Board("1n1R2k1/2b1qpp1/p3p2p/1p6/1P2Q2P/4PNP1/P4PB1/6K1 b - - 0 1")
#         self.assertFalse(board.is_pinned(zhchess.BLACK, zhchess.B8))

#         # The empty square e8 would be considered pinned.
#         self.assertTrue(board.is_pinned(zhchess.BLACK, zhchess.E8))

#     def test_impossible_en_passant(self):
#         # Not a pawn there.
#         board = zhchess.Board("1b1b4/8/b1P5/2kP4/8/2b4K/8/8 w - c6 0 1")
#         self.assertTrue(board.status() & zhchess.STATUS_INVALID_EP_SQUARE)

#         # Sixth rank square not empty.
#         board = zhchess.Board("5K2/8/2pp2Pp/2PP4/P5Pp/2pP1Ppp/P6p/7k b - g3 0 1")
#         self.assertTrue(board.status() & zhchess.STATUS_INVALID_EP_SQUARE)

#         # Seventh rank square not empty.
#         board = zhchess.Board("8/7k/8/7p/8/8/8/K7 w - h6 0 1")
#         self.assertTrue(board.status() & zhchess.STATUS_INVALID_EP_SQUARE)

#     def test_horizontally_skewered_en_passant(self):
#         # Horizontal pin. Non-evasion.
#         board = zhchess.Board("8/8/8/r2Pp2K/8/8/4k3/8 w - e6 0 1")
#         move = zhchess.Move.from_uci("d5e6")
#         self.assertEqual(board.status(), zhchess.STATUS_VALID)
#         self.assertTrue(board.is_pseudo_legal(move))
#         self.assertIn(move, board.generate_pseudo_legal_moves())
...
#         board = zhchess.Board("2b1r2r/8/5P1k/2p1pP2/5R1P/6PK/4q3/4R3 w - e6 0 1")
#         move = zhchess.Move.from_uci("f5e6")
...
#         board = zhchess.Board("8/8/8/5k2/4Pp2/8/2B5/4K3 b - e3 0 1")
#         move = zhchess.Move.from_uci("f4e3")
#         self.assertTrue(board.is_pseudo_legal(move))
#         self.assertIn(move, board.generate_pseudo_legal_moves())
#         self.assertIn(move, board.generate_pseudo_legal_ep())
#         self.assertFalse(board.is_legal(move))
...
#         board = zhchess.Board("8/8/8/7B/6Pp/8/4k2K/3r4 b - g3 0 1")
#         move = zhchess.Move.from_uci("h4g3")
#         self.assertTrue(board.is_pseudo_legal(move))
#         self.assertIn(move, board.generate_pseudo_legal_moves())
#         self.assertIn(move, board.generate_pseudo_legal_ep())
#         self.assertFalse(board.is_legal(move))
...
#         board = zhchess.Board("8/5K2/8/3k4/3pP3/8/8/3R4 b - e3 0 1")
#         move = zhchess.Move.from_uci("d4e3")
#         self.assertTrue(board.is_pseudo_legal(move))
#         self.assertIn(move, board.generate_pseudo_legal_moves())
#         self.assertIn(move, board.generate_pseudo_legal_ep())
#         self.assertFalse(board.is_legal(move))
...
#         board = zhchess.Board("8/8/8/2k5/2pP4/8/4K3/8 b - d3 0 1")
#         move = zhchess.Move.from_uci("c4d3")
...
#         board = zhchess.Board("3q1rk1/ppp1p1pp/4b3/3pPp2/3P4/1K1n4/PPQ2PPP/3b1BNR w - f6 0 1")
...
#         self.assertIn(zhchess.Move.from_uci("c2c7"), plc)
#         self.assertIn(zhchess.Move.from_uci("c2d3"), plc)
#         self.assertEqual(len(plc), 5)

#     def test_castling_is_legal(self):
#         board = zhchess.Board("rnbqkbnr/5p2/1pp3pp/p2P4/6P1/2NPpN2/PPP1Q1BP/R3K2R w Qq - 0 11")
#         self.assertFalse(board.is_legal(zhchess.Move.from_uci("e1g1")))
#         self.assertFalse(board.is_legal(zhchess.Move.from_uci("e1h1")))

#         board.castling_rights |= zhchess.BB_H1
#         self.assertTrue(board.is_legal(zhchess.Move.from_uci("e1g1")))
#         self.assertTrue(board.is_legal(zhchess.Move.from_uci("e1h1")))

#     def test_from_chess960_pos(self):
#         board = zhchess.Board.from_chess960_pos(909)
#         self.assertTrue(board.chess960)
#         self.assertEqual(board.fen(), "rkqbrnbn/pppppppp/8/8/8/8/PPPPPPPP/RKQBRNBN w KQkq - 0 1")

#     def test_mirror(self):
#         board = zhchess.Board("r1bq1r2/pp2n3/4N2k/3pPppP/1b1n2Q1/2N5/PP3PP1/R1B1K2R w KQ g6 0 15")
#         mirrored = zhchess.Board("r1b1k2r/pp3pp1/2n5/1B1N2q1/3PpPPp/4n2K/PP2N3/R1BQ1R2 b kq g3 0 15")
...
#         self.assertEqual(board.board_fen(), zhchess.STARTING_BOARD_FEN)
...
#         board = zhchess.Board("r3k2r/8/8/8/8/8/8/R3K2R w Qkq - 0 1")
...
#         self.assertFalse(board.is_irreversible(zhchess.Move.null()))
...
#         board = zhchess.Board("8/8/8/B2p3Q/2qPp1P1/b7/2P2PkP/4K2R b K - 0 1")
...
#         board = zhchess.Board("rnbqk1nr/bb3p1p/1q2r3/2pPp3/3P4/7P/1PP1NpPP/R1BQKBNR w KQkq c6")
#         self.assertEqual(board.status(), zhchess.STATUS_IMPOSSIBLE_CHECK)
#         self.assertEqual(board.ep_square, zhchess.C6)
...
#         self.assertEqual(len(list(zhchess.Board().legal_moves)), 20)

#     def test_nonzero(self):
#         self.assertTrue(zhchess.Board().legal_moves)
#         self.assertTrue(zhchess.Board().pseudo_legal_moves)

#         caro_kann_mate = zhchess.Board("r1bqkb1r/pp1npppp/2pN1n2/8/3P4/8/PPP1QPPP/R1B1KBNR b KQkq - 4 6")
#         self.assertFalse(caro_kann_mate.legal_moves)
#         self.assertTrue(caro_kann_mate.pseudo_legal_moves)

#     def test_string_conversion(self):
#         board = zhchess.Board("r3k1nr/ppq1pp1p/2p3p1/8/1PPR4/2N5/P3QPPP/5RK1 b kq b3 0 16")
...
#         board = MockBoard()
#         gen = zhchess.LegalMoveGenerator(board)
#         list(gen)
...
#     def test_equality(self):
#         a1 = zhchess.SquareSet(zhchess.BB_RANK_4)
#         a2 = zhchess.SquareSet(zhchess.BB_RANK_4)
#         b1 = zhchess.SquareSet(zhchess.BB_RANK_5 | zhchess.BB_RANK_6)
#         b2 = zhchess.SquareSet(zhchess.BB_RANK_5 | zhchess.BB_RANK_6)
...
#         self.assertEqual(zhchess.SquareSet(zhchess.BB_ALL), zhchess.BB_ALL)
#         self.assertEqual(zhchess.BB_ALL, zhchess.SquareSet(zhchess.BB_ALL))

#         self.assertEqual(int(zhchess.SquareSet(zhchess.SquareSet(999))), 999)
#         self.assertEqual(zhchess.SquareSet([zhchess.B8]), zhchess.BB_B8)
...
#         bb = zhchess.SquareSet(zhchess.BB_H8 | zhchess.BB_B7 | zhchess.BB_RANK_1)
#         self.assertEqual(str(bb), expected)

#     def test_iter(self):
#         bb = zhchess.SquareSet(zhchess.BB_G7 | zhchess.BB_G8)
#         self.assertEqual(list(bb), [zhchess.G7, zhchess.G8])

#     def test_reversed(self):
#         bb = zhchess.SquareSet(zhchess.BB_A1 | zhchess.BB_B1 | zhchess.BB_A7 | zhchess.BB_E1)
#         self.assertEqual(list(reversed(bb)), [zhchess.A7, zhchess.E1, zhchess.B1, zhchess.A1])

#     def test_arithmetic(self):
#         self.assertEqual(zhchess.SquareSet(zhchess.BB_RANK_2) & zhchess.BB_FILE_D, zhchess.BB_D2)
#         self.assertEqual(zhchess.SquareSet(zhchess.BB_ALL) ^ zhchess.BB_EMPTY, zhchess.BB_ALL)
#         self.assertEqual(zhchess.SquareSet(zhchess.BB_C1) | zhchess.BB_FILE_C, zhchess.BB_FILE_C)

#         bb = zhchess.SquareSet(zhchess.BB_EMPTY)
#         bb ^= zhchess.BB_ALL
#         self.assertEqual(bb, zhchess.BB_ALL)
#         bb &= zhchess.BB_E4
#         self.assertEqual(bb, zhchess.BB_E4)
#         bb |= zhchess.BB_RANK_4
#         self.assertEqual(bb, zhchess.BB_RANK_4)

#         self.assertEqual(zhchess.SquareSet(zhchess.BB_F3) << 1, zhchess.BB_G3)
#         self.assertEqual(zhchess.SquareSet(zhchess.BB_C8) >> 2, zhchess.BB_A8)

#         bb = zhchess.SquareSet(zhchess.BB_D1)
#         bb <<= 1
#         self.assertEqual(bb, zhchess.BB_E1)
#         bb >>= 2
#         self.assertEqual(bb, zhchess.BB_C1)
...
#         for a in examples:
#             self.assertEqual(zhchess.SquareSet(a).copy(), a)

#         for a in examples:
#             a = zhchess.SquareSet(a)
#             for b in examples:
#                 b = zhchess.SquareSet(b)
...
#         squares = zhchess.SquareSet(zhchess.BB_A1)
#         squares.update(zhchess.BB_FILE_H)
#         self.assertEqual(squares, zhchess.BB_A1 | zhchess.BB_FILE_H)

#         squares.intersection_update(zhchess.BB_RANK_8)
#         self.assertEqual(squares, zhchess.BB_H8)

#         squares.difference_update(zhchess.BB_A1)
#         self.assertEqual(squares, zhchess.BB_H8)

#         squares.symmetric_difference_update(zhchess.BB_A1)
#         self.assertEqual(squares, zhchess.BB_A1 | zhchess.BB_H8)

#         squares.add(zhchess.A3)
#         self.assertEqual(squares, zhchess.BB_A1 | zhchess.BB_A3 | zhchess.BB_H8)

#         squares.remove(zhchess.H8)
#         self.assertEqual(squares, zhchess.BB_A1 | zhchess.BB_A3)

#         with self.assertRaises(KeyError):
#             squares.remove(zhchess.H8)

#         squares.discard(zhchess.H8)

#         squares.discard(zhchess.A1)
#         self.assertEqual(squares, zhchess.BB_A3)

#         squares.clear()
#         self.assertEqual(squares, zhchess.BB_EMPTY)

#         with self.assertRaises(KeyError):
#             squares.pop()

#         squares.add(zhchess.C7)
#         self.assertEqual(squares.pop(), zhchess.C7)
#         self.assertEqual(squares, zhchess.BB_EMPTY)

#     def test_from_square(self):
#         self.assertEqual(zhchess.SquareSet.from_square(zhchess.H5), zhchess.BB_H5)
#         self.assertEqual(zhchess.SquareSet.from_square(zhchess.C2), zhchess.BB_C2)

#     def test_carry_rippler(self):
#         self.assertEqual(sum(1 for _ in zhchess.SquareSet(zhchess.BB_D1).carry_rippler()), 2 ** 1)
#         self.assertEqual(sum(1 for _ in zhchess.SquareSet(zhchess.BB_FILE_B).carry_rippler()), 2 ** 8)

#     def test_mirror(self):
#         self.assertEqual(zhchess.SquareSet(0x00a2_0900_0004_a600).mirror(), 0x00a6_0400_0009_a200)
#         self.assertEqual(zhchess.SquareSet(0x1e22_2212_0e0a_1222).mirror(), 0x2212_0a0e_1222_221e)

#     def test_flip(self):
#         self.assertEqual(zhchess.flip_vertical(zhchess.BB_ALL), zhchess.BB_ALL)
#         self.assertEqual(zhchess.flip_horizontal(zhchess.BB_ALL), zhchess.BB_ALL)
#         self.assertEqual(zhchess.flip_diagonal(zhchess.BB_ALL), zhchess.BB_ALL)
#         self.assertEqual(zhchess.flip_anti_diagonal(zhchess.BB_ALL), zhchess.BB_ALL)

#         s = zhchess.SquareSet(0x1e22_2212_0e0a_1222)  # Letter R
#         self.assertEqual(zhchess.flip_vertical(s), 0x2212_0a0e_1222_221e)
#         self.assertEqual(zhchess.flip_horizontal(s), 0x7844_4448_7050_4844)
#         self.assertEqual(zhchess.flip_diagonal(s), 0x0000_6192_8c88_ff00)
#         self.assertEqual(zhchess.flip_anti_diagonal(s), 0x00ff_1131_4986_0000)

#     def test_len_of_complenent(self):
#         squares = zhchess.SquareSet(~zhchess.BB_ALL)
#         self.assertEqual(len(squares), 0)

#         squares = ~zhchess.SquareSet(zhchess.BB_BACKRANKS)
#         self.assertEqual(len(squares), 48)

#     def test_int_conversion(self):
#         self.assertEqual(int(zhchess.SquareSet(zhchess.BB_CENTER)), 0x0000_0018_1800_0000)
#         self.assertEqual(hex(zhchess.SquareSet(zhchess.BB_CENTER)), "0x1818000000")
#         self.assertEqual(bin(zhchess.SquareSet(zhchess.BB_CENTER)), "0b1100000011000000000000000000000000000")

#     def test_tolist(self):
#         self.assertEqual(zhchess.SquareSet(zhchess.BB_LIGHT_SQUARES).tolist().count(True), 32)

#     def test_flip_ducktyping(self):
#         bb = 0x1e22_2212_0e0a_1222
#         squares = zhchess.SquareSet(bb)
#         for f in [zhchess.flip_vertical, zhchess.flip_horizontal, zhchess.flip_diagonal, zhchess.flip_anti_diagonal]:
...
#         with zhchess.polyglot.open_reader("data/polyglot/performance.bin") as book:
#             pos = zhchess.Board()
...
#         with zhchess.polyglot.open_reader("data/polyglot/lasker-trap.bin") as book:
#             board = zhchess.Board("rnbqk1nr/ppp2ppp/8/4P3/1BP5/8/PP2KpPP/RN1Q1BNR b kq - 1 7")
...
#             pos = zhchess.Board("r1bqr1k1/pp1nbppp/2p2n2/3p2B1/3P4/2NBP3/PPQ1NPPP/R3K2R w KQ - 5 10")
...
#             # Exchange Variation.
#             pos = zhchess.Board("r3k1nr/1pp1q1pp/p1pb1p2/4p3/3PP1b1/2P1BN2/PP1N1PPP/R2Q1RK1 b kq - 4 9")
#             moves = set(entry.move for entry in book.find_all(pos))
#             self.assertIn(pos.parse_san("O-O-O"), moves)
#             self.assertEqual(len(moves), 1)

#             # Not a castling move.
#             pos = zhchess.Board("1r1qr1k1/1b2bp1n/p2p2pB/1pnPp2p/P1p1P3/R1P2NNP/1PBQ1PP1/4R1K1 w - - 0 1")
#             entry = book.find(pos)
#             self.assertEqual(entry.move, zhchess.Move.from_uci("e1a1"))

#     def test_empty_book(self):
#         with zhchess.polyglot.open_reader(os.devnull) as book:
#             self.assertEqual(len(book), 0)

#             entries = book.find_all(zhchess.Board())
...
#         with zhchess.polyglot.open_reader("data/polyglot/performance.bin") as book:
#             # Uniform choice.
#             entry = book.choice(zhchess.Board(), random=FirstMockRandom())
#             self.assertEqual(entry.move, zhchess.Move.from_uci("e2e4"))

#             entry = book.choice(zhchess.Board(), random=LastMockRandom())
#             self.assertEqual(entry.move, zhchess.Move.from_uci("c2c4"))

#             # Weighted choice.
#             entry = book.weighted_choice(zhchess.Board(), random=FirstMockRandom())
#             self.assertEqual(entry.move, zhchess.Move.from_uci("e2e4"))

#             entry = book.weighted_choice(zhchess.Board(), random=LastMockRandom())
#             self.assertEqual(entry.move, zhchess.Move.from_uci("c2c4"))

#             # Weighted choice with excluded move.
#             entry = book.weighted_choice(zhchess.Board(), exclude_moves=[zhchess.Move.from_uci("e2e4")], random=FirstMockRandom())
#             self.assertEqual(entry.move, zhchess.Move.from_uci("d2d4"))

#     def test_find(self):
#         with zhchess.polyglot.open_reader("data/polyglot/performance.bin") as book:
#             entry = book.find(zhchess.Board())
#             self.assertEqual(entry.move, zhchess.Move.from_uci("e2e4"))

#     def test_exclude_moves(self):
#         with zhchess.polyglot.open_reader("data/polyglot/performance.bin") as book:
#             entry = book.find(zhchess.Board(), exclude_moves=[zhchess.Move.from_uci("e2e4")])
#             self.assertEqual(entry.move, zhchess.Move.from_uci("d2d4"))
...
#                 book.find(zhchess.Board(), minimum_weight=2)
...
#         e4_h5.nags.add(zhchess.pgn.NAG_MISTAKE)
...
#         e4_e5_Qf3.nags.add(zhchess.pgn.NAG_MISTAKE)
...
#         exporter = zhchess.pgn.StringExporter(headers=False, comments=False, variations=False)
#         game.accept(exporter)
#         self.assertEqual(str(exporter), "1. e4 d5 2. exd5 *")

#         exporter = zhchess.pgn.StringExporter(headers=False, comments=False)
#         game.accept(exporter)
#         self.assertEqual(str(exporter), "1. e4 d5 ( 1... h5 ) ( 1... e5 2. Qf3 ) ( 1... c5 ) 2. exd5 *")

#         exporter = zhchess.pgn.StringExporter()
#         game.accept(exporter)
#         pgn = textwrap.dedent("""\
#             [Event "?"]
#             [Site "?"]
...
#         exporter = zhchess.pgn.FileExporter(virtual_file)
#         game.accept(exporter)
#         self.assertEqual(virtual_file.getvalue(), pgn + "\n\n")

#     def test_game_without_tag_roster(self):
#         game = zhchess.pgn.Game.without_tag_roster()
#         self.assertEqual(str(game), "*")

#     def test_setup(self):
#         game = zhchess.pgn.Game()
#         self.assertEqual(game.board(), zhchess.Board())
...
#         game.setup(zhchess.STARTING_FEN)
#         self.assertNotIn("FEN", game.headers)
#         self.assertNotIn("SetUp", game.headers)
#         self.assertNotIn("Variant", game.headers)
...
#         e4 = zhchess.Move.from_uci("e2e4")
#         d4 = zhchess.Move.from_uci("d2d4")

#         node = zhchess.pgn.Game()
...
#             third_game = zhchess.pgn.read_game(pgn)
#             fourth_game = zhchess.pgn.read_game(pgn)
#             fifth_game = zhchess.pgn.read_game(pgn)
#             sixth_game = zhchess.pgn.read_game(pgn)
#             self.assertTrue(zhchess.pgn.read_game(pgn) is None)
...
#         node = game
#         while node.variations and not node.has_variation(zhchess.Move.from_uci("a7a6")):
#             node = node[0]
...
#         self.assertEqual(game.end().board(), zhchess.Board("8/2p2k2/1pR3p1/1P1P4/p1P2P2/P4K2/8/5r2 w - - 7 78"))
...
#         game = zhchess.pgn.read_game(pgn)
#         self.assertEqual(game[0].move, zhchess.Move.from_uci("e2e4"))
...
#         node = game[0]
#         self.assertEqual(node.move, zhchess.Move.from_uci("e2e4"))
#         self.assertFalse(node.comment)
#         self.assertFalse(node.starting_comment)

#         node = game[1]
#         self.assertEqual(node.move, zhchess.Move.from_uci("d2d4"))
#         self.assertFalse(node.comment)
...
#         node = game.variation(zhchess.Move.from_uci("b2b4"))
#         self.assertIn(zhchess.pgn.NAG_DUBIOUS_MOVE, node.nags)
...
#         node = node[0]
#         self.assertIn(zhchess.pgn.NAG_MISTAKE, node.nags)
#         self.assertEqual(len(node.nags), 1)

#         node = node[0]
#         self.assertIn(zhchess.pgn.NAG_BRILLIANT_MOVE, node.nags)
#         self.assertEqual(len(node.nags), 1)

#     def test_tree_traversal(self):
#         game = zhchess.pgn.Game()
#         node = game.add_variation(zhchess.Move(zhchess.E2, zhchess.E4))
#         alternative_node = game.add_variation(zhchess.Move(zhchess.D2, zhchess.D4))
#         end_node = node.add_variation(zhchess.Move(zhchess.E7, zhchess.E5))
...
#         game = zhchess.pgn.Game()
#         a = game.add_variation(zhchess.Move(zhchess.A2, zhchess.A3))
#         b = game.add_variation(zhchess.Move(zhchess.B2, zhchess.B3))
...
#         c = game.add_main_variation(zhchess.Move(zhchess.C2, zhchess.C3))
...
#         pgn.seek(offsets[0])
#         self.assertEqual(zhchess.pgn.read_game(pgn).next().move, zhchess.Move.from_uci("a2a3"))
#         pgn.seek(offsets[1])
#         self.assertEqual(zhchess.pgn.read_game(pgn).next().move, zhchess.Move.from_uci("b2b3"))
#         pgn.seek(offsets[2])
#         self.assertEqual(zhchess.pgn.read_game(pgn).next().move, zhchess.Move.from_uci("d2d3"))
#         self.assertEqual(zhchess.pgn.read_game(pgn), None)
...
#             pgn.seek(offsets[0])
#             first_drawn_game = zhchess.pgn.read_game(pgn)
#             self.assertEqual(first_drawn_game.headers["Site"], "03")
#             self.assertEqual(first_drawn_game[0].move, zhchess.Move.from_uci("d2d3"))

#     def test_visit_board(self):
#         class TraceVisitor(zhchess.pgn.BaseVisitor):
#             def __init__(self):
#                 self.trace = []
...
#         self.assertEqual(trace, zhchess.pgn.read_game(pgn, Visitor=TraceVisitor))

#         pgn.seek(0)
#         self.assertEqual(trace, zhchess.pgn.read_game(pgn).accept(TraceVisitor()))

#         pgn.seek(0)
#         self.assertEqual(zhchess.Board(trace[-1]), zhchess.pgn.read_game(pgn, Visitor=zhchess.pgn.BoardBuilder))

#     def test_black_to_move(self):
#         game = zhchess.pgn.Game()
#         game.setup("8/8/4k3/8/4P3/4K3/8/8 b - - 0 17")
#         node = game
#         node = node.add_main_variation(zhchess.Move.from_uci("e6d6"))
#         node = node.add_main_variation(zhchess.Move.from_uci("e3d4"))
#         node = node.add_main_variation(zhchess.Move.from_uci("d6e6"))
...
#         board = zhchess.Board("rbb1N1k1/pp1n1ppp/8/2Pp4/3P4/4P3/P1Q2PPq/R1BR1K2 b - - 0 1")
...
#             self.assertEqual(game.next().move, zhchess.Move.from_uci("e2e4"))

#             self.assertTrue(zhchess.pgn.read_game(pgn) is None)

#     def test_game_from_board(self):
#         setup = "3k4/8/4K3/8/8/8/8/2R5 b - - 0 1"
#         board = zhchess.Board(setup)
#         board.push_san("Ke8")
#         board.push_san("Rc8#")

#         game = zhchess.pgn.Game.from_board(board)
#         self.assertEqual(game.headers["FEN"], setup)

#         end_node = game.end()
#         self.assertEqual(end_node.move, zhchess.Move.from_uci("c1c8"))
#         self.assertEqual(end_node.parent.move, zhchess.Move.from_uci("d8e8"))
...
#         game = zhchess.pgn.Game()
#         game.add_variation(zhchess.Move.from_uci("e2e4"))

#         moves = [zhchess.Move.from_uci("g1f3"), zhchess.Move.from_uci("d7d5")]

#         tail = game.add_line(moves, starting_comment="start", comment="end", nags=(17, 42))

#         self.assertEqual(tail.parent.move, zhchess.Move.from_uci("g1f3"))
...
#         self.assertEqual(tail.move, zhchess.Move.from_uci("d7d5"))
#         self.assertEqual(tail.comment, "end")
#         self.assertIn(42, tail.nags)

#     def test_mainline(self):
#         moves = [zhchess.Move.from_uci(uci) for uci in ["d2d3", "g8f6", "e2e4"]]
...
#         game = zhchess.pgn.read_game(pgn)
#         self.assertEqual(game.end().move, zhchess.Move.from_uci("e2e4"))
...
#         game.setup(zhchess.variant.SuicideBoard())
#         self.assertEqual(game.headers["Variant"], "Suicide")

#         game.setup(zhchess.Board())
#         self.assertNotIn("Variant", game.headers)
...
#         subgame = node.accept_subgame(zhchess.pgn.GameBuilder())
#         self.assertEqual(subgame.headers["FEN"], "rnbqkb1r/pppppppp/5n2/8/3P4/8/PPP1PPPP/RNBQKBNR w KQkq - 1 2")
#         self.assertEqual(subgame.next().move, zhchess.Move.from_uci("c2c4"))
#         self.assertEqual(subgame.variations[1].move, zhchess.Move.from_uci("g1f3"))

#     def test_is_wild(self):
#         headers = zhchess.pgn.Headers()
#         headers["Variant"] = "wild/1"
#         self.assertTrue(headers.is_wild())

#     def test_my_game_node(self):
#         class MyGameNode(zhchess.pgn.GameNode):
...
#         class MyChildNode(zhchess.pgn.ChildNode, MyGameNode):
#             pass

#         class MyGame(zhchess.pgn.Game, MyGameNode):
#             pass

#         pgn = io.StringIO("1. e4")
#         game = zhchess.pgn.read_game(pgn, Visitor=MyGame.builder)
#         self.assertTrue(isinstance(game, MyGame))
#         node = game.variation(zhchess.Move.from_uci("e2e4"))
#         self.assertTrue(isinstance(node, MyGameNode))

#     def test_recursion(self):
#         board = zhchess.Board("4k3/8/8/8/8/8/8/4K3 w - - 0 1")
#         for _ in range(1000):
#             board.push(zhchess.Move(zhchess.E1, zhchess.E2))
#             board.push(zhchess.Move(zhchess.E8, zhchess.E7))
#             board.push(zhchess.Move(zhchess.E2, zhchess.E1))
#             board.push(zhchess.Move(zhchess.E7, zhchess.E8))
#         game = zhchess.pgn.Game.from_board(board)
...
#         game.set_arrows([(zhchess.A1, zhchess.A1), zhchess.svg.Arrow(zhchess.A1, zhchess.H1, color="red"), zhchess.svg.Arrow(zhchess.B1, zhchess.B8)])
...
#         # Driven by parser.
#         game = zhchess.pgn.read_game(io.StringIO(pgn), Visitor=BlackVariationsOnly)
#         self.assertEqual(game.accept(zhchess.pgn.StringExporter(headers=False)), expected_pgn)

#         # Driven by game tree traversal.
#         game = zhchess.pgn.read_game(io.StringIO(pgn)).accept(BlackVariationsOnly())
#         self.assertEqual(game.accept(zhchess.pgn.StringExporter(headers=False)), expected_pgn)
...
#         a = zhchess.engine.UciOptionMap()
#         b = zhchess.engine.UciOptionMap()
#         c = zhchess.engine.UciOptionMap()
#         self.assertEqual(a, b)
...
#         order = [
#             zhchess.engine.Mate(-0),
#             zhchess.engine.Mate(-1),
#             zhchess.engine.Mate(-99),
#             zhchess.engine.Cp(-123),
#             zhchess.engine.Cp(-50),
#             zhchess.engine.Cp(0),
#             zhchess.engine.Cp(+30),
#             zhchess.engine.Cp(+800),
#             zhchess.engine.Mate(+77),
#             zhchess.engine.Mate(+1),
#             zhchess.engine.MateGiven,
#         ]
...
#     def test_score(self):
#         # Negation.
#         self.assertEqual(-zhchess.engine.Cp(+20), zhchess.engine.Cp(-20))
#         self.assertEqual(-zhchess.engine.Mate(+4), zhchess.engine.Mate(-4))
#         self.assertEqual(-zhchess.engine.Mate(-0), zhchess.engine.MateGiven)
#         self.assertEqual(-zhchess.engine.MateGiven, zhchess.engine.Mate(-0))

#         # Score.
#         self.assertEqual(zhchess.engine.Cp(-300).score(), -300)
#         self.assertEqual(zhchess.engine.Mate(+5).score(), None)
#         self.assertEqual(zhchess.engine.Mate(+5).score(mate_score=100000), 99995)
#         self.assertEqual(zhchess.engine.Mate(-7).score(mate_score=100000), -99993)

#         # Mate.
#         self.assertEqual(zhchess.engine.Cp(-300).mate(), None)
#         self.assertEqual(zhchess.engine.Mate(+5).mate(), 5)

#         # Wdl.
#         self.assertEqual(zhchess.engine.MateGiven.wdl().expectation(), 1)
#         self.assertEqual(zhchess.engine.Mate(0).wdl().expectation(), 0)
#         self.assertEqual(zhchess.engine.Cp(0).wdl().expectation(), 0.5)

#         for cp in map(zhchess.engine.Cp, range(-1050, 1100, 50)):
#             wdl = cp.wdl()
#             self.assertTrue(wdl)
#             self.assertAlmostEqual(wdl.winning_chance() + wdl.drawing_chance() + wdl.losing_chance(), 1)

#         self.assertFalse(zhchess.engine.Wdl(0, 0, 0))

#     def test_wdl_model(self):
#         self.assertEqual(zhchess.engine.Cp(131).wdl(model="sf12", ply=25), zhchess.engine.Wdl(524, 467, 9))
#         self.assertEqual(zhchess.engine.Cp(146).wdl(model="sf14", ply=25), zhchess.engine.Wdl(601, 398, 1))
#         self.assertEqual(zhchess.engine.Cp(40).wdl(model="sf15", ply=25), zhchess.engine.Wdl(58, 937, 5))
#         self.assertEqual(zhchess.engine.Cp(100).wdl(model="sf15.1", ply=64), zhchess.engine.Wdl(497, 503, 0))

#     @catchAndSkip(FileNotFoundError, "need stockfish")
#     def test_sf_forced_mates(self):
#         with zhchess.engine.SimpleEngine.popen_uci("stockfish", debug=True) as engine:
#             epds = [
#                 "1k1r4/pp1b1R2/3q2pp/4p3/2B5/4Q3/PPP2B2/2K5 b - - bm Qd1+; id \"BK.01\";",
...
#                 result = engine.play(board, zhchess.engine.Limit(mate=5), game=object())
...
#         with zhchess.engine.SimpleEngine.popen_uci("stockfish", setpgrp=True, debug=True) as engine:
#             board = zhchess.Board("8/6K1/1p1B1RB1/8/2Q5/2n1kP1N/3b4/4n3 w - - 0 1")
#             limit = zhchess.engine.Limit(depth=40)
...
#                     if "score" in info and info["score"].white() >= zhchess.engine.Mate(+2):
#                         break

#             analysis.wait()
#             self.assertFalse(analysis.would_block())

#             self.assertEqual(analysis.info["score"].relative, zhchess.engine.Mate(+2))
#             self.assertEqual(analysis.multipv[0]["score"].black(), zhchess.engine.Mate(-2))
...
#         with zhchess.engine.SimpleEngine.popen_uci("stockfish", debug=True) as engine:
#             board = zhchess.Board("r2qr1k1/pb2npp1/1pn1p2p/8/3P4/P1PQ1N2/B4PPP/R1B1R1K1 w - - 2 15")
#             result = engine.analyse(board, zhchess.engine.Limit(depth=1), multipv=3)
...
#         engine = zhchess.engine.SimpleEngine.popen_uci("stockfish", setpgrp=True, debug=True)

#         with engine:
#             engine.quit()

#         with self.assertRaises(zhchess.engine.EngineTerminatedError), engine:
#             engine.ping()
...
#                 with zhchess.engine.SimpleEngine.popen_xboard("crafty", setpgrp=True, debug=True, cwd=tmpdir) as engine:
#                     board = zhchess.Board("2bqkbn1/2pppp2/np2N3/r3P1p1/p2N2B1/5Q2/PPPPKPP1/RNB2r2 w KQkq - 0 1")
#                     limit = zhchess.engine.Limit(depth=10)
...
#         try:
#             with tempfile.TemporaryDirectory(prefix="crafty") as tmpdir:
#                 with zhchess.engine.SimpleEngine.popen_xboard("crafty", debug=True, cwd=tmpdir) as engine:
#                     board = zhchess.Board("2bqkbn1/2pppp2/np2N3/r3P1p1/p2N2B1/5Q2/PPPPKPP1/RNB2r2 w KQkq - 0 1")
#                     limit = zhchess.engine.Limit(depth=7, time=2.0)
#                     info = engine.analyse(board, limit)
#                     self.assertTrue(info["score"].relative > zhchess.engine.Cp(1000))
#                     engine.quit()
#         finally:
#             logging.disable(logging.NOTSET)
...
#         async def main():
#             protocol = zhchess.engine.UciProtocol()
#             mock = zhchess.engine.MockTransport(protocol)
...
#             board = zhchess.Board()
#             result = await protocol.play(board, zhchess.engine.Limit(time=0.123),
#                                          root_moves=[board.parse_san("e4"), board.parse_san("d4")],
#                                          ponder=True,
#                                          info=zhchess.engine.INFO_ALL)
#             self.assertEqual(result.move, zhchess.Move.from_uci("d2d4"))
#             self.assertEqual(result.ponder, zhchess.Move.from_uci("d7d5"))
...
#             limit = zhchess.engine.Limit(white_clock=0.001, black_clock=0.002,
...
#             board = zhchess.Board()
#             board.push_uci("d2d4")
#             with self.assertRaises(zhchess.engine.EngineError):
#                 await protocol.play(board, zhchess.engine.Limit(time=5.0))
#             mock.assert_done()
...
#             result = await protocol.analysis(zhchess.Board())
#             self.assertTrue(result.would_block())
#             result.stop()
#             best = await result.wait()
#             self.assertFalse(result.would_block())
#             self.assertEqual(best.move, zhchess.Move.from_uci("e2e4"))
...
#             best = await result.wait()
#             self.assertEqual(best.move, zhchess.Move.from_uci("e2e4"))
#             self.assertEqual(best.ponder, zhchess.Move.from_uci("e7e5"))
#             mock.assert_done()
...
#             info = await protocol.analyse(board, zhchess.engine.Limit(depth=20))
#             self.assertEqual(info, {})

#             # Play.
#             mock.expect("position startpos")
#             mock.expect("go movetime 3000", ["bestmove a2a4 ponder a7a5"])
#             await protocol.play(board, zhchess.engine.Limit(time=3))
...
#             primary_opponent = zhchess.engine.Opponent("Eliza", None, 3500, True)
...
#             board = zhchess.Board()
#             result = await protocol.play(board, zhchess.engine.Limit(time=1), ponder=True)
#             self.assertEqual(result.move, zhchess.Move.from_uci("d2d4"))
#             self.assertEqual(result.ponder, zhchess.Move.from_uci("g8f6"))
...
#             result = await protocol.play(board, zhchess.engine.Limit(time=2), ponder=True)
#             self.assertEqual(result.move, zhchess.Move.from_uci("c2c4"))
#             self.assertEqual(result.ponder, zhchess.Move.from_uci("e7e6"))
...
#             result = await protocol.play(board, zhchess.engine.Limit(time=3), ponder=True, options={"Hash": 32})
#             self.assertEqual(result.move, zhchess.Move.from_uci("b1c3"))
#             self.assertEqual(result.ponder, zhchess.Move.from_uci("f8b4"))
...
#             mock.expect("go ponder movetime 3000")
#             result = await protocol.play(board, zhchess.engine.Limit(time=3), ponder=True)
#             self.assertEqual(result.move, zhchess.Move.from_uci("d1c2"))
#             self.assertEqual(result.ponder, zhchess.Move.from_uci("d7d5"))
...
#             await protocol.analyse(board, zhchess.engine.Limit(time=4))
...
#             await protocol.play(board, zhchess.engine.Limit(time=5), ponder=True)

#             # Ponderhit prevented by new opponent, which starts a new game.
#             board.push(zhchess.Move.from_uci("c4d5"))
#             board.push(zhchess.Move.from_uci("e6d5"))
...
#             mock.expect("go ponder movetime 5000")
#             opponent = zhchess.engine.Opponent("Guy Chapman", "GM", 3000, False)
#             await protocol.play(board, zhchess.engine.Limit(time=5), ponder=True, opponent=opponent)

#             # Ponderhit prevented by restoration of previous opponent, which again starts a new game.
#             board.push(zhchess.Move.from_uci("c1g5"))
#             board.push(zhchess.Move.from_uci("h7h6"))
...
#         # Info: refutation.
#         board = zhchess.Board("8/8/6k1/8/8/8/1K6/3B4 w - - 0 1")
#         info = zhchess.engine._parse_uci_info("refutation d1h5 g6h5", board)
#         self.assertEqual(info["refutation"][zhchess.Move.from_uci("d1h5")], [zhchess.Move.from_uci("g6h5")])

#         info = zhchess.engine._parse_uci_info("refutation d1h5", board)
#         self.assertEqual(info["refutation"][zhchess.Move.from_uci("d1h5")], [])

#         # Info: string.
#         info = zhchess.engine._parse_uci_info("string goes to end no matter score cp 4 what", board)
#         self.assertEqual(info["string"], "goes to end no matter score cp 4 what")

#         # Info: currline.
#         info = zhchess.engine._parse_uci_info("currline 0 e2e4 e7e5", zhchess.Board())
#         self.assertEqual(info["currline"][0], [zhchess.Move.from_uci("e2e4"), zhchess.Move.from_uci("e7e5")])

#         # Info: ebf.
#         info = zhchess.engine._parse_uci_info("ebf 0.42", board)
...
#         # Unknown tokens.
#         board = zhchess.Board()
#         info = zhchess.engine._parse_uci_info("depth 1 unkown1 seldepth 2 unknown2 time 16 nodes 1 score cp 72 unknown3 wdl 249 747 4 multipv 1 uknown4 pv g1f3 g8f6 unknown5", board)
...
#         info = zhchess.engine._parse_uci_info("depth 1 seldepth 2 time 16 nodes 1 score cp 72 wdl 249 747 4 hashfull 0 nps 400 tbhits 0 multipv 1", board)
...
#             ])
#             result = await protocol.play(zhchess.Board("QN4n1/6r1/3k4/8/b2K4/8/8/8 b - - 0 1"), zhchess.engine.Limit(), info=zhchess.engine.INFO_ALL)
#             self.assertEqual(result.move, zhchess.Move.from_uci("g7g4"))
#             self.assertEqual(result.ponder, zhchess.Move.from_uci("d4c3"))
#             self.assertEqual(result.info["pv"], [zhchess.Move.from_uci("g7g4"), zhchess.Move.from_uci("d4c3")])
...
#         async def main():
#             protocol = zhchess.engine.XBoardProtocol()
#             mock = zhchess.engine.MockTransport(protocol)
...
#             limit = zhchess.engine.Limit(time=1.5, depth=17)
#             board = zhchess.Board()
#             board.push_san("d4")
#             board.push_san("Nf6")
#             board.push_san("c4")
...
#             limit = zhchess.engine.Limit(time=5)
#             board = zhchess.Board()
#             opponent = zhchess.engine.Opponent("Turk", "Mechanical", 2100, True)
...
#             new_opponent = zhchess.engine.Opponent("Turochamp", None, 800, True)
#             board.push(result.move)
#             mock.expect("new")
...
#             board = zhchess.variant.AtomicBoard("rnbqkbnr/pppppppp/8/8/8/5N2/PPPPPPPP/RNBQKB1R b KQkq - 1 1")
#             limit = zhchess.engine.Limit(depth=1)
#             mock.expect("new")
...
#             self.assertEqual(info["score"], zhchess.engine.PovScore(zhchess.engine.Cp(116), zhchess.BLACK))
#             self.assertEqual(info["time"], 0.23)
#             self.assertEqual(info["nodes"], 2252)
#             self.assertEqual(info["pv"], [zhchess.Move.from_uci(move) for move in ["f7f6", "e2e4", "e7e6"]])
#             mock.assert_done()
...
#             limit = zhchess.engine.Limit(black_clock=65, white_clock=100,
...
#             mock.expect("go", ["move e2e4"])
#             mock.expect_ping()
#             result = await protocol.play(zhchess.Board(), limit)
#             self.assertEqual(result.move, zhchess.Move.from_uci("e2e4"))
#             mock.assert_done()
...
#         async def main():
#             transport, protocol = await zhchess.engine.popen_uci(["/bin/bash", "-c", "read && echo uciok && sleep 86400"])
...
#         with zhchess.engine.SimpleEngine.popen_uci(["/bin/bash", "-c", "read && echo uciok && sleep 86400"], debug=True) as engine:
#             engine.timeout = 0.01
...
#             zhchess.engine.run_in_background(raise_expected_error)
...
#         result = zhchess.engine.run_in_background(resolve)
...
#         board = zhchess.Board("8/8/8/5N2/5K2/2kB4/8/8 b - - 0 1")
#         key_from_board = zhchess.syzygy.calc_key(board)
#         key_from_filename = zhchess.syzygy.normalize_tablename("KBNvK")
#         self.assertEqual(key_from_board, key_from_filename)

#     def test_tablenames(self):
#         self.assertIn("KPPvKN", zhchess.syzygy.tablenames())
#         self.assertIn("KNNPvKN", zhchess.syzygy.tablenames())
#         self.assertIn("KQRNvKR", zhchess.syzygy.tablenames())
#         self.assertIn("KRRRvKR", zhchess.syzygy.tablenames())
#         self.assertIn("KRRvKRR", zhchess.syzygy.tablenames())
#         self.assertIn("KRNvKRP", zhchess.syzygy.tablenames())
#         self.assertIn("KRPvKP", zhchess.syzygy.tablenames())

#     def test_suicide_tablenames(self):
#         # Test the number of 6-piece tables.
#         self.assertEqual(sum(1 for eg in zhchess.syzygy.tablenames(one_king=False) if len(eg) == 7), 5754)

#     def test_normalize_tablename(self):
#         names = set(zhchess.syzygy.tablenames())
#         for name in names:
#             self.assertTrue(
#                 zhchess.syzygy.normalize_tablename(name) in names,
#                 f"Already normalized {name}")

#             w, b = name.split("v", 1)
#             swapped = b + "v" + w
#             self.assertTrue(
#                 zhchess.syzygy.normalize_tablename(swapped) in names,
#                 f"Normalized {swapped}")

#     def test_normalize_nnvbb(self):
#         self.assertEqual(zhchess.syzygy.normalize_tablename("KNNvKBB"), "KBBvKNN")

#     def test_dependencies(self):
#         self.assertEqual(set(zhchess.syzygy.dependencies("KBNvK")), set(["KBvK", "KNvK"]))

#     def test_get_wdl_get_dtz(self):
#         with zhchess.syzygy.Tablebase() as tables:
#             board = zhchess.Board()
#             self.assertEqual(tables.get_dtz(board, tables.get_wdl(board)), None)

#     def test_probe_pawnless_wdl_table(self):
#         wdl = zhchess.syzygy.WdlTable("data/syzygy/regular/KBNvK.rtbw")
#         wdl.init_table_wdl()

#         board = zhchess.Board("8/8/8/5N2/5K2/2kB4/8/8 b - - 0 1")
#         self.assertEqual(wdl.probe_wdl_table(board), -2)

#         board = zhchess.Board("7B/5kNK/8/8/8/8/8/8 w - - 0 1")
#         self.assertEqual(wdl.probe_wdl_table(board), 2)

#         board = zhchess.Board("N7/8/2k5/8/7K/8/8/B7 w - - 0 1")
#         self.assertEqual(wdl.probe_wdl_table(board), 2)

#         board = zhchess.Board("8/8/1NkB4/8/7K/8/8/8 w - - 1 1")
#         self.assertEqual(wdl.probe_wdl_table(board), 0)

#         board = zhchess.Board("8/8/8/2n5/2b1K3/2k5/8/8 w - - 0 1")
#         self.assertEqual(wdl.probe_wdl_table(board), -2)

#         wdl.close()

#     def test_probe_wdl_table(self):
#         wdl = zhchess.syzygy.WdlTable("data/syzygy/regular/KRvKP.rtbw")
#         wdl.init_table_wdl()

#         board = zhchess.Board("8/8/2K5/4P3/8/8/8/3r3k b - - 1 1")
#         self.assertEqual(wdl.probe_wdl_table(board), 0)

#         board = zhchess.Board("8/8/2K5/8/4P3/8/8/3r3k b - - 1 1")
#         self.assertEqual(wdl.probe_wdl_table(board), 2)

#         wdl.close()

#     def test_probe_dtz_table_piece(self):
#         dtz = zhchess.syzygy.DtzTable("data/syzygy/regular/KRvKN.rtbz")
#         dtz.init_table_dtz()

#         # Pawnless position with white to move.
#         board = zhchess.Board("7n/6k1/4R3/4K3/8/8/8/8 w - - 0 1")
#         self.assertEqual(dtz.probe_dtz_table(board, 2), (0, -1))

#         # Same position with black to move.
#         board = zhchess.Board("7n/6k1/4R3/4K3/8/8/8/8 b - - 1 1")
#         self.assertEqual(dtz.probe_dtz_table(board, -2), (8, 1))

#         dtz.close()

#     def test_probe_dtz_table_pawn(self):
#         dtz = zhchess.syzygy.DtzTable("data/syzygy/regular/KNvKP.rtbz")
#         dtz.init_table_dtz()

#         board = zhchess.Board("8/1K6/1P6/8/8/8/6n1/7k w - - 0 1")
#         self.assertEqual(dtz.probe_dtz_table(board, 2), (2, 1))

#         dtz.close()

#     def test_probe_wdl_tablebase(self):
#         with zhchess.syzygy.Tablebase(max_fds=2) as tables:
#             self.assertGreaterEqual(tables.add_directory("data/syzygy/regular"), 70)

#             # Winning KRvKB.
#             board = zhchess.Board("7k/6b1/6K1/8/8/8/8/3R4 b - - 12 7")
#             self.assertEqual(tables.probe_wdl_table(board), -2)

#             # Drawn KBBvK.
#             board = zhchess.Board("7k/8/8/4K3/3B4/4B3/8/8 b - - 12 7")
#             self.assertEqual(tables.probe_wdl_table(board), 0)

#             # Winning KBBvK.
#             board = zhchess.Board("7k/8/8/4K2B/8/4B3/8/8 w - - 12 7")
#             self.assertEqual(tables.probe_wdl_table(board), 2)

#     def test_wdl_ep(self):
#         with zhchess.syzygy.open_tablebase("data/syzygy/regular") as tables:
#             # Winning KPvKP because of en passant.
#             board = zhchess.Board("8/8/8/k2Pp3/8/8/8/4K3 w - e6 0 2")
...
#         with zhchess.syzygy.open_tablebase("data/syzygy/regular") as tables:
#             board = zhchess.Board("8/8/8/8/2pP4/2K5/4k3/8 b - d3 0 1")
#             self.assertEqual(tables.probe_dtz_no_ep(board), -1)
#             self.assertEqual(tables.probe_dtz(board), 1)

#     def test_testsuite(self):
#         with zhchess.syzygy.open_tablebase("data/syzygy/regular") as tables, open("data/endgame.epd") as epds:
#             board = zhchess.Board()
...
#     @catchAndSkip(zhchess.syzygy.MissingTableError)
#     def test_stockfish_dtz_bug(self):
#         with zhchess.syzygy.open_tablebase("data/syzygy/regular") as tables:
#             board = zhchess.Board("3K4/8/3k4/8/4p3/4B3/5P2/8 w - - 0 5")
#             self.assertEqual(tables.probe_dtz(board), 15)

#     @catchAndSkip(zhchess.syzygy.MissingTableError)
#     def test_issue_93(self):
#         with zhchess.syzygy.open_tablebase("data/syzygy/regular") as tables:
#             board = zhchess.Board("4r1K1/6PP/3k4/8/8/8/8/8 w - - 1 64")
#             self.assertEqual(tables.probe_wdl(board), 2)
#             self.assertEqual(tables.probe_dtz(board), 4)

#     @catchAndSkip(zhchess.syzygy.MissingTableError)
#     def test_suicide_dtm(self):
#         with zhchess.syzygy.open_tablebase("data/syzygy/suicide", VariantBoard=zhchess.variant.SuicideBoard) as tables, open("data/suicide-dtm.epd") as epds:
#             for epd in epds:
#                 epd = epd.strip()

#                 board, solution = zhchess.variant.SuicideBoard.from_epd(epd)
...
#                 if wdl > 0:
#                     self.assertGreaterEqual(dtz, zhchess.syzygy.dtz_before_zeroing(wdl))
#                     self.assertLessEqual(dtz, 2 * solution["max_dtm"])
#                 elif wdl == 0:
#                     self.assertEqual(dtz, 0)
#                 else:
#                     self.assertLessEqual(dtz, zhchess.syzygy.dtz_before_zeroing(wdl))
#                     self.assertGreaterEqual(dtz, 2 * solution["max_dtm"])

#     @catchAndSkip(zhchess.syzygy.MissingTableError)
#     def test_suicide_dtz(self):
#         with zhchess.syzygy.open_tablebase("data/syzygy/suicide", VariantBoard=zhchess.variant.SuicideBoard) as tables, open("data/suicide-dtz.epd") as epds:
#             for epd in epds:
#                 epd = epd.strip()
...
#         board = zhchess.variant.SuicideBoard()

#         with zhchess.syzygy.open_tablebase("data/syzygy/suicide", VariantBoard=type(board)) as tables, open("data/suicide-stats.epd") as epds:
...
#         kvk = zhchess.variant.AntichessBoard("4k3/8/8/8/8/8/8/4K3 w - - 0 1")

#         tables = zhchess.syzygy.Tablebase()
#         with self.assertRaises(KeyError):
#             tables.probe_dtz(kvk)

#         tables = zhchess.syzygy.Tablebase(VariantBoard=zhchess.variant.AntichessBoard)
#         with self.assertRaises(zhchess.syzygy.MissingTableError):
#             tables.probe_dtz(kvk)
...
#     def setUp(self):
#         self.tablebase = zhchess.gaviota.open_tablebase_native("data/gaviota")

#     def tearDown(self):
#         self.tablebase.close()

#     def test_native_probe_dtm(self):
#         board = zhchess.Board("6K1/8/8/8/4Q3/8/6k1/8 b - - 0 1")
#         self.assertEqual(self.tablebase.probe_dtm(board), -14)

#         board = zhchess.Board("8/3K4/8/8/8/4r3/4k3/8 b - - 0 1")
#         self.assertEqual(self.tablebase.get_dtm(board), 21)

#     def test_native_probe_wdl(self):
#         board = zhchess.Board("8/8/4K3/2n5/8/3k4/8/8 w - - 0 1")
#         self.assertEqual(self.tablebase.probe_wdl(board), 0)

#         board = zhchess.Board("8/8/1p2K3/8/8/3k4/8/8 b - - 0 1")
#         self.assertEqual(self.tablebase.get_wdl(board), 1)

#     @catchAndSkip(zhchess.gaviota.MissingTableError, "need KPPvKP.gtb.cp4")
#     def test_two_ep(self):
#         board = zhchess.Board("8/8/8/8/5pPp/8/5K1k/8 b - g3 0 61")
...
#     def setUp(self):
#         self.tablebase = zhchess.gaviota.open_tablebase("data/gaviota", LibraryLoader=None)

#     def tearDown(self):
#         self.tablebase.close()

#     @catchAndSkip(zhchess.gaviota.MissingTableError)
#     def test_dm_4(self):
...
#                 # Parse EPD.
#                 board, extra = zhchess.Board.from_epd(epd)
...
#         board = zhchess.Board("K7/8/8/6k1/5pPp/8/8/8 b - g3 0 61")
...
#         svg = zhchess.BaseBoard("4k3/8/8/8/8/8/8/4KB2")._repr_svg_()
#         self.assertIn("white bishop", svg)
#         self.assertNotIn("black queen", svg)

#     def test_svg_arrows(self):
#         svg = zhchess.svg.board(arrows=[(zhchess.A1, zhchess.A1)])
#         self.assertIn("<circle", svg)
#         self.assertNotIn("<line", svg)

#         svg = zhchess.svg.board(arrows=[zhchess.svg.Arrow(zhchess.A1, zhchess.H8)])
#         self.assertNotIn("<circle", svg)
#         self.assertIn("<line", svg)

#     def test_svg_piece(self):
#         svg = zhchess.svg.piece(zhchess.Piece.from_symbol("K"))
...
#         board = zhchess.variant.SuicideBoard("4k3/8/8/8/8/1r3B2/8/4K3 b - - 0 1")
#         Rxf3 = board.parse_san("Rxf3")
#         Rb4 = zhchess.Move.from_uci("b3b4")
...
#         # Kings only.
#         board = zhchess.variant.SuicideBoard("8/8/8/2k5/8/8/4K3/8 b - - 0 1")
#         self.assertFalse(board.is_insufficient_material())

#         # Bishops on the same color.
#         board = zhchess.variant.SuicideBoard("8/8/8/5b2/2B5/1B6/8/8 b - - 0 1")
#         self.assertFalse(board.is_insufficient_material())

#         # Opposite-color bishops.
#         board = zhchess.variant.SuicideBoard("4b3/8/8/8/3B4/2B5/8/8 b - - 0 1")
#         self.assertTrue(board.is_insufficient_material())

#         # Pawn not blocked.
#         board = zhchess.variant.SuicideBoard("8/5b2/5P2/8/3B4/2B5/8/8 b - - 0 1")
#         self.assertFalse(board.is_insufficient_material())

#         # Pawns blocked, but on wrong color.
#         board = zhchess.variant.SuicideBoard("8/5p2/5P2/8/8/8/3b4/8 b - - 0 1")
#         self.assertFalse(board.is_insufficient_material())

#         # Stalemate.
#         board = zhchess.variant.SuicideBoard("6B1/6pB/6P1/8/8/8/8/8 b - - 0 1")
#         self.assertFalse(board.is_insufficient_material())

#         # Pawns not really locked up.
#         board = zhchess.variant.SuicideBoard("8/8/8/2pp4/2PP4/8/8/8 w - - 0 1")
#         self.assertFalse(board.is_insufficient_material())

#     def test_king_promotions(self):
#         board = zhchess.variant.SuicideBoard("8/6P1/8/3K1k2/8/8/3p4/8 b - - 0 1")
#         d1K = zhchess.Move.from_uci("d2d1k")
...
#         board = zhchess.variant.AtomicBoard(fen)
#         board.push_san("dxc4")
...
#         board = zhchess.variant.AtomicBoard("8/8/1Q2pk2/8/8/8/3K4/1n6 w - - 0 1")
#         self.assertTrue(board.is_check())
#         Qa7 = zhchess.Move.from_uci("b6a7")
...
#         board = zhchess.variant.AtomicBoard("8/8/8/2K5/2P5/2k1n3/8/2R5 b - - 0 1")
#         Nxc4 = zhchess.Move.from_uci("e3c4")
...
#         # Real-world position.
#         board = zhchess.variant.AtomicBoard("rn2kb1r/2p1p2p/p2q1pp1/1pPP4/Q7/4P3/PP3P1P/R3K3 w Qkq b6 0 11")
#         board.push_san("cxb6+")
#         self.assertEqual(board.fen(), "rn2kb1r/2p1p2p/p2q1pp1/3P4/Q7/4P3/PP3P1P/R3K3 b Qkq - 0 11")

#         # Test the explosion radius.
#         board = zhchess.variant.AtomicBoard("3kK3/8/8/2NNNNN1/2NN1pN1/2NN1NN1/2NNPNN1/2NNNNN1 w - - 0 1")
#         board.push_san("e4")
#         board.push_san("fxe3")
...
#         # Starting position.
#         board = zhchess.variant.AtomicBoard()
#         self.assertFalse(board.is_insufficient_material())

#         # Single rook.
#         board = zhchess.variant.AtomicBoard("8/3k4/8/8/4R3/4K3/8/8 w - - 0 1")
#         self.assertTrue(board.is_insufficient_material())

#         # Only bishops, but no captures possible.
#         board = zhchess.variant.AtomicBoard("7k/4b3/8/8/8/3B4/2B5/K7 w - - 0 1")
#         self.assertTrue(board.is_insufficient_material())

#         # Bishops of both sides on the same color complex.
#         board = zhchess.variant.AtomicBoard("7k/3b4/8/8/8/3B4/2B5/K7 w - - 0 1")
#         self.assertFalse(board.is_insufficient_material())

#         # Knights on both sides.
#         board = zhchess.variant.AtomicBoard("8/1n6/1k6/8/8/8/3KN3/8 w - - 0 1")
#         self.assertFalse(board.is_insufficient_material())

#         # Two knights can not win.
#         board = zhchess.variant.AtomicBoard("8/1nn5/1k6/8/8/8/3K4/8 w - - 0 1")
#         self.assertTrue(board.is_insufficient_material())

#         # KQN can win (even KQ could).
#         board = zhchess.variant.AtomicBoard("3Q4/5kKB/8/8/8/8/8/8 b - - 0 1")
#         self.assertFalse(board.is_insufficient_material())

#     def test_castling_uncovered_rank_attack(self):
#         board = zhchess.variant.AtomicBoard("8/8/8/8/8/8/4k3/rR4KR w KQ - 0 1", chess960=True)
#         self.assertFalse(board.is_legal(zhchess.Move.from_uci("g1b1")))

#         # Kings are touching at the end.
#         board = zhchess.variant.AtomicBoard("8/8/8/8/8/8/2k5/rR4KR w KQ - 0 1", chess960=True)
#         self.assertTrue(board.is_legal(zhchess.Move.from_uci("g1b1")))

#     def test_atomic_castle_with_kings_touching(self):
#         board = zhchess.variant.AtomicBoard("5b1r/1p5p/4ppp1/4Bn2/1PPP1PP1/4P2P/3k4/4K2R w K - 1 1")
#         board.push_san("O-O")
#         self.assertEqual(board.fen(), "5b1r/1p5p/4ppp1/4Bn2/1PPP1PP1/4P2P/3k4/5RK1 b - - 2 1")

#         board = zhchess.variant.AtomicBoard("8/8/8/8/8/8/4k3/R3K2q w Q - 0 1")
#         board.push_san("O-O-O")
#         self.assertEqual(board.fen(), "8/8/8/8/8/8/4k3/2KR3q b - - 1 1")

#         board = zhchess.variant.AtomicBoard("8/8/8/8/8/8/5k2/R3K2r w Q - 0 1")
#         with self.assertRaises(zhchess.IllegalMoveError):
#             board.push_san("O-O-O")

#         board = zhchess.variant.AtomicBoard("8/8/8/8/8/8/6k1/R5Kr w Q - 0 1", chess960=True)
#         with self.assertRaises(zhchess.IllegalMoveError):
#             board.push_san("O-O-O")

#         board = zhchess.variant.AtomicBoard("8/8/8/8/8/8/4k3/r2RK2r w D - 0 1", chess960=True)
#         with self.assertRaises(zhchess.IllegalMoveError):
#             board.push_san("O-O-O")

#     def test_castling_rights_explode_with_king(self):
#         board = zhchess.variant.AtomicBoard("rnb1kbnr/pppppppp/8/4q3/8/8/PPPPPPPP/RNBQKBNR b KQkq - 0 1")
#         board.push_san("Qxe2#")
#         self.assertEqual(board.fen(), "rnb1kbnr/pppppppp/8/8/8/8/PPPP1PPP/RNB3NR w kq - 0 2")
#         self.assertEqual(board.castling_rights, zhchess.BB_A8 | zhchess.BB_H8)

#     def test_lone_king_wdl(self):
#         tables = zhchess.syzygy.Tablebase(VariantBoard=zhchess.variant.AtomicBoard)
#         board = zhchess.variant.AtomicBoard.empty()
#         board.set_piece_at(zhchess.D1, zhchess.Piece.from_symbol("k"))
#         self.assertEqual(tables.probe_wdl(board), -2)
...
#         board = zhchess.variant.RacingKingsBoard("1K6/7k/8/8/8/8/8/8 b - - 0 1")
#         self.assertFalse(board.is_game_over())
...
#         board = zhchess.variant.RacingKingsBoard("1k6/6K1/8/8/8/8/8/8 w - - 0 1")
#         self.assertFalse(board.is_variant_draw())
#         self.assertFalse(board.is_variant_win())
#         self.assertTrue(board.is_variant_loss())

#         # Black to move is lost, because they cannot reach the backrank.
#         board = zhchess.variant.RacingKingsBoard("5RK1/1k6/8/8/8/8/8/8 b - - 0 1")
#         self.assertFalse(board.is_variant_draw())
#         self.assertFalse(board.is_variant_win())
#         self.assertTrue(board.is_variant_loss())

#         # White far away.
#         board = zhchess.variant.RacingKingsBoard("k1q1R2Q/3N4/8/8/5K2/6n1/1b6/1r6 w - - 4 19")
...
#         board = zhchess.variant.RacingKingsBoard("2KR4/k7/2Q5/4q3/8/8/8/2N5 b - - 0 1")
#         self.assertTrue(board.is_variant_end())
#         self.assertTrue(board.is_variant_loss())
#         self.assertFalse(board.is_variant_win())
#         self.assertFalse(board.is_variant_draw())
...
#         board = zhchess.variant.RacingKingsBoard("1r4RK/6R1/k1r5/8/8/8/4N3/q2n1n2 b - - 0 1")
#         self.assertTrue(board.is_variant_end())
#         self.assertTrue(board.is_variant_loss())
#         self.assertFalse(board.is_variant_win())
#         self.assertFalse(board.is_variant_draw())
...
#         board = zhchess.variant.RacingKingsBoard("kr3NK1/1q2R3/8/8/8/5n2/2N5/1rb2B1R w - - 11 14")
#         self.assertTrue(board.is_variant_end())
#         self.assertFalse(board.is_variant_loss())
#         self.assertFalse(board.is_variant_win())
...
#         # Another draw.
#         board = zhchess.variant.RacingKingsBoard("1knq1RK1/2n5/8/8/3N4/6N1/6B1/8 w - - 23 25")
#         self.assertTrue(board.is_variant_end())
#         self.assertFalse(board.is_variant_loss())
#         self.assertFalse(board.is_variant_win())
#         self.assertTrue(board.is_variant_draw())
...
#         board = zhchess.variant.RacingKingsBoard("1Q4R1/5K2/4B3/8/8/3N4/8/k7 b - - 0 1")
#         self.assertTrue(board.is_game_over())
#         self.assertTrue(board.is_stalemate())
#         self.assertFalse(board.is_variant_win())
#         self.assertFalse(board.is_variant_draw())
...
#         board = zhchess.variant.RacingKingsBoard("4Q1K1/8/7k/4R3/8/5B2/8/3N4 b - - 0 1")
#         self.assertTrue(board.is_game_over())
#         self.assertFalse(board.is_stalemate())
#         self.assertFalse(board.is_variant_win())
#         self.assertTrue(board.is_variant_loss())
...
#         self.assertTrue(zhchess.variant.RacingKingsBoard().is_valid())

#         # This position with black to move and both kings on the backrank is
#         # invalid because the race should have been over already.
#         board = zhchess.variant.RacingKingsBoard("3krQK1/8/8/8/1q6/3B1N2/1b6/1R4R1 b - - 0 0")
#         self.assertEqual(board.status(), zhchess.STATUS_RACE_OVER)

#     def test_race_material(self):
#         board = zhchess.variant.RacingKingsBoard()

#         # Switch color of the black rook.
#         board.set_piece_at(zhchess.B1, zhchess.Piece.from_symbol("R"))
#         self.assertEqual(board.status(), zhchess.STATUS_RACE_MATERIAL)

#     def test_legal_moves_after_end(self):
#         board = zhchess.variant.RacingKingsBoard("1k5b/5b2/8/8/8/8/3N3K/N4B2 w - - 0 1")
#         self.assertTrue(board.is_variant_end())
#         self.assertTrue(board.is_variant_loss())
#         self.assertFalse(board.is_stalemate())
...
#         board = zhchess.variant.RacingKingsBoard("8/8/8/8/R7/8/krbnNB1K/qrbnNBRQ b - - 1 1")
#         self.assertFalse(board.is_valid())
#         self.assertEqual(board.status(), zhchess.STATUS_RACE_CHECK | zhchess.STATUS_TOO_MANY_CHECKERS | zhchess.STATUS_IMPOSSIBLE_CHECK)


# class HordeTestCase(unittest.TestCase):

#     def test_status(self):
#         board = zhchess.variant.HordeBoard()
#         self.assertEqual(board.status(), zhchess.STATUS_VALID)

#         # Black (non-horde) piece on first rank.
#         board = zhchess.variant.HordeBoard("rnb1kbnr/ppp1pppp/2Pp2PP/1P3PPP/PPP1PPPP/PPP1PPPP/PPP1PPP1/PPPqPP2 w kq - 0 1")
#         self.assertEqual(board.status(), zhchess.STATUS_VALID)

#     def test_double_pawn_push(self):
#         board = zhchess.variant.HordeBoard("8/8/8/8/8/3k1p2/8/PPPPPPPP w - - 0 1")

#         # Double pawn push blocked by king.
#         self.assertNotIn(zhchess.Move.from_uci("d1d3"), board.generate_legal_moves())

#         # Double pawn push from backrank possible.
#         self.assertIn(zhchess.Move.from_uci("e1e2"), board.generate_legal_moves())
#         self.assertTrue(board.is_legal(board.parse_san("e2")))
#         self.assertIn(zhchess.Move.from_uci("e1e3"), board.generate_legal_moves())
...
#         board = zhchess.variant.ThreeCheckBoard()
#         self.assertEqual(board.fen(), zhchess.variant.ThreeCheckBoard.starting_fen)
...
#         self.assertEqual(board.fen(), zhchess.variant.ThreeCheckBoard(lichess_fen).fen())

#     def test_copy(self):
#         fen = "8/8/1K2p3/3qP2k/8/8/8/8 b - - 2+1 3 57"
#         board = zhchess.variant.ThreeCheckBoard(fen)
...
#         board, extra = zhchess.variant.ThreeCheckBoard.from_epd(epd)
...
#         a = zhchess.variant.ThreeCheckBoard()
#         a.push_san("e4")

#         b = zhchess.variant.ThreeCheckBoard("rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq - 0 1 +0+0")
#         c = zhchess.variant.ThreeCheckBoard("rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq - 0 1 +0+1")
...
#         self.assertEqual(ops["bm"], [zhchess.Move.from_uci("h4f2")])


# class CrazyhouseTestCase(unittest.TestCase):

#     def test_pawn_drop(self):
#         board = zhchess.variant.CrazyhouseBoard("r2q1rk1/ppp2pp1/1bnp3p/3B4/3PP1b1/4PN2/PP4PP/R2Q1RK1[BNPnp] b - - 0 13")
#         P_at_e6 = zhchess.Move.from_uci("P@e6")
#         self.assertIn(zhchess.E6, board.legal_drop_squares())
...
#             game = zhchess.pgn.read_game(pgn)
#             self.assertEqual(game.end().board().move_stack[23], zhchess.Move.from_uci("N@f3"))

#     def test_pawns_in_pocket(self):
#         board = zhchess.variant.CrazyhouseBoard("r2q1rk1/ppp2pp1/1bnp3p/3Bp3/4P1b1/2PPPN2/PP4PP/R2Q1RK1/NBn w - - 22 12")
#         board.push_san("d4")
#         board.push_san("exd4")
#         board.push_san("cxd4")
...
#         board = zhchess.variant.CrazyhouseBoard("4k3/8/8/1n6/8/3B4/8/4K3 w - - 0 1")
#         board.push_san("Bxb5+")
...
#         board = zhchess.variant.CrazyhouseBoard("4k3/8/8/8/8/8/1p6/2R1K3 b - - 0 1")
...
#         self.assertEqual(board.pop(), zhchess.Move.from_uci("e5d6"))
#         self.assertEqual(board.fen(), fen)
...
#         board = zhchess.variant.CrazyhouseBoard("r3k2r/8/8/8/8/8/8/R3K2R w Qkq - 0 1")
#         self.assertTrue(board.is_irreversible(board.parse_san("Ra2")))
#         self.assertTrue(board.is_irreversible(board.parse_san("O-O-O")))
#         self.assertTrue(board.is_irreversible(board.parse_san("Kd1")))
#         self.assertTrue(board.is_irreversible(board.parse_san("Rxa8")))
