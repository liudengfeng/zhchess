...
def perft(depth: int, board: zhchess.Board) -> int:
    if depth == 1:
...
def parallel_perft(pool: multiprocessing.pool.Pool, depth: int, board: zhchess.Board) -> int:
    if depth == 1:
        return board.legal_moves.count()
    elif depth > 1:
        def successors(board: zhchess.Board) -> Iterator[zhchess.Board]:
            for move in board.legal_moves:
...
def main(perft_file: TextIO, VariantBoard: Type[zhchess.Board], perft_f: Callable[[int, zhchess.Board], int], max_depth: Optional[int], max_nodes: Optional[int]) -> None:
    current_id = None
