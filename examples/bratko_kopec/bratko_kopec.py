...
def parse_epd(epd: str, VariantBoard: Type[zhchess.Board]) -> Tuple[zhchess.Board, str, List[zhchess.Move], List[zhchess.Move]]:
    board, epd_info = VariantBoard.from_epd(epd)

    description = str(epd_info.get("id", board.fen()))

    if "am" in epd_info:
        am = typing.cast(List[zhchess.Move], epd_info["am"])
        description = "{} (avoid {})".format(description, " and ".join(board.san(m) for m in am))
    else:
        am = []

    if "bm" in epd_info:
        bm = typing.cast(List[zhchess.Move], epd_info["bm"])
...
async def test_epd(engine: zhchess.engine.Protocol, epd: str, VariantBoard: Type[zhchess.Board], movetime: float) -> float:
    board, description, am, bm = parse_epd(epd, VariantBoard)

    limit = zhchess.engine.Limit(time=movetime)
...
async def test_epd_with_fractional_scores(engine: zhchess.engine.Protocol, epd: str, VariantBoard: Type[zhchess.Board], movetime: float) -> float:
    board, description, am, bm = parse_epd(epd, VariantBoard)
...
async def main() -> None:
    ...
    # Find variant.
    VariantBoard = zhchess.variant.find_variant(args.variant)

    # Open and configure engine.
    engine: zhchess.engine.Protocol
    if args.uci:
        _, engine = await zhchess.engine.popen_uci(args.uci)
        if args.threads > 1:
            await engine.configure({"Threads": args.threads})
    else:
        _, engine = await zhchess.engine.popen_xboard(args.xboard)
        if args.threads > 1:
